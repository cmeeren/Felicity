...
module Article =
    ...
    let createdAt = define.Attribute.SimpleDateTimeOffset().Get(fun a -> a.CreatedAt)
    ...
    let updatedAt =
        define.Attribute.Nullable.SimpleDateTimeOffset().Get(fun a -> a.UpdatedAt)
...
module Person =
    ...
    let id =
        define.Id.ParsedOpt(PersonId.toString, PersonId.fromString, (fun a -> a.Id))
    ...
    let articles =
        define.Relationship.ToMany(Article.resourceDef).GetAsync(Db.Article.allByAuthor)

    let getCollection =
        define.Operation.GetCollection(fun ctx parser ->
            parser
                .For(PersonSearchArgs.empty)
    ...
    let patch = define.Operation.Patch().AfterUpdateAsync(Db.Person.save)

    let delete = define.Operation.DeleteAsync(Db.Person.delete)
...
module Comment =
    ...
    let id =
        define.Id.ParsedOpt(CommentId.toString, CommentId.fromString, (fun a -> a.Id))
