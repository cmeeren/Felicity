...
let invalidContentType () =
    ...
    |> Error.setDetail
        $"Request content must be sent with Content-Type set to the JSON:API media type (%s{Constants.jsonApiMediaType})"
...
let invalidContentTypeParams () =
    ...
    |> Error.setDetail
        "The JSON:API media type in the Content-Type header must not be modified with media type parameters"
...
let incorrectPathCase invalidPath expectedPath =
    ...
    |> Error.setDetail
        $"The path '%s{invalidPath}' does not exist, but differs only by case from the existing path '%s{expectedPath}'. Paths are case sensitive."
...
let preconditionRequired hasETag hasLastModified =
    let msg =
        ...
        | true, true ->
            "This operation requires a precondition to be specified using the If-Match or If-Unmodified-Since headers"
...
let preconditionFailed hasETag hasLastModified =
    ...
    Error.create 412 |> Error.setTitle "Precondition failed" |> Error.setDetail msg
...
let reqParserMissingIncludedResource resType resId pointer =
    ...
    |> Error.setDetail
        $"Expected to find referenced resource with type '%s{resType}' and ID '%s{resId}' in the 'included' part of the request"
    |> Error.setSourcePointer pointer
...
let invalidParsedNone (info: ParsedValueInfo) =
    ...
    | FromQuery data ->
        if data.NumValues > 1 then
            ...
            |> Error.setDetail
                $"Comma-separated query parameter '%s{data.Name}' got invalid value '%s{data.Value}' for item %i{data.ValueIndex + 1}"
...
let invalidParsedErrMsg (info: ParsedValueInfo) errMsg =
    ...
    | FromQuery data ->
        if data.NumValues > 1 then
            ...
            |> Error.setTitle "Invalid query parameter value"
            |> Error.setDetail
                $"Comma-separated query parameter '%s{data.Name}' got invalid value '%s{data.Value}' for item %i{data.ValueIndex + 1}: %s{errMsg}"
            |> Error.setSourceParam data.Name
        else
            Error.create 400
            |> Error.setTitle "Invalid query parameter value"
...
let invalidEnum (info: ParsedValueInfo) allowedValues =
    ...
    | FromQuery data ->
        ...
        if data.NumValues > 1 then
            ...
            |> Error.setTitle "Invalid query parameter value"
            |> Error.setDetail
                $"Comma-separated query parameter '%s{data.Name}' got invalid value '%s{data.Value}' for item %i{data.ValueIndex + 1}; expected %s{expectedStr}"
            |> Error.setSourceParam data.Name
        else
            Error.create 400
            |> Error.setTitle "Invalid query parameter value"
            |> Error.setDetail
                $"Query parameter '%s{data.Name}' got invalid value '%s{data.Value}'; expected %s{expectedStr}"
            |> Error.setSourceParam data.Name
    | FromHeader data ->
        Error.create 400
        |> Error.setTitle "Invalid header value"
...
let queryNotSingular paramName numValues =
    ...
    |> Error.setDetail
        $"Query parameter '%s{paramName}' only accepts a single value, but got %i{numValues} comma-separated values"
    |> Error.setSourceParam paramName
...
let setToManyRelReplacementNotSupported relName resType pointer supportsPost supportsDelete =
    ...
    |> Error.setDetail
        $"Relationship '%s{relName}' on type '%s{resType}' does not support complete replacement using PATCH%s{extraMessage}"
    |> Error.setSourcePointer pointer
...
let set2OneFieldMissing presentFieldName missingFieldName =
    ...
    |> Error.setDetail
        $"Field '%s{presentFieldName}' can only be set together with field '%s{missingFieldName}', which was missing"
...
let collPostClientIdNotAllowed collName resType =
    ...
    |> Error.setDetail
        $"Collection '%s{collName}' does not support creating resource type '%s{resType}' with a client-generated ID"
...
let collPostTypeNotAllowed collName invalidType allowedTypes pointer =
    ...
    |> match allowedTypes with
       | [ x ] ->
           Error.setDetail
               $"Collection '%s{collName}' does not support creating resources with type '%s{invalidType}'; expected '%s{x}'"
       | xs ->
           Error.setDetailf
               "Collection '%s' does not support creating resources with type '%s'; expected one of %s"
               collName
               invalidType
...
let resGetNotSupportedPolymorphic resType collName =
    ...
    |> Error.setDetail
        $"GET is not supported for resource type '%s{resType}' (it may be supported for other resource types in collection '%s{collName}')"
...
let resPatchNotSupportedPolymorphic resType collName =
    ...
    |> Error.setDetail
        $"PATCH is not supported for resource type '%s{resType}' (it may be supported for other resource types in collection '%s{collName}')"
...
let resDeleteNotSupportedPolymorphic resType collName =
    ...
    |> Error.setDetail
        $"DELETE is not supported for resource type '%s{resType}' (it may be supported for other resource types in collection '%s{collName}')"
...
let relNotDefinedPolymorphic relName resType collName =
    ...
    |> Error.setDetail
        $"Relationship '%s{relName}' is not defined for resource type '%s{resType}' (other resource types in collection '%s{collName}' may have a relationship called '%s{relName}')"
...
let getRelNotDefinedPolymorphic relName resType collName =
    ...
    |> Error.setDetail
        $"Relationship '%s{relName}' on type '%s{resType}' is not readable (other resource types in collection '%s{collName}' may have a readable relationship called '%s{relName}')"
...
let relInvalidType relName invalidType allowedTypes pointer =
    ...
    |> match allowedTypes with
       | [ x ] ->
           Error.setDetail
               $"Relationship '%s{relName}' contains data with invalid type '%s{invalidType}'; expected '%s{x}'"
       | xs ->
           Error.setDetailf
               "Relationship '%s' contains data with invalid type '%s'; expected one of %s"
               relName
               invalidType
               (xs |> List.map (sprintf "'%s'") |> String.concat ", ")
    |> Error.setSourcePointer pointer
...
let relInvalidTypeSelf invalidType allowedTypes pointer =
    ...
    |> match allowedTypes with
       ...
       | xs ->
           Error.setDetailf
               "Data contains invalid type '%s'; expected one of %s"
               invalidType
               (xs |> List.map (sprintf "'%s'") |> String.concat ", ")
    |> Error.setSourcePointer pointer
...
let patchToManyRelSelfNotAllowedPolymorphic relName resType supportsPost supportsDelete collName =
    ...
    |> Error.setTitle "Complete replacement not allowed"
    |> Error.setDetail
        $"Relationship '%s{relName}' on type '%s{resType}' does not support complete replacement using PATCH%s{extraMessage} (other resource types in collection '%s{collName}' may have a relationship called '%s{relName}' that supports PATCH)"
...
let patchRelSelfSettableButNotGettablePolymorphic relName resType collName =
    ...
    |> Error.setDetail
        $"Relationship '%s{relName}' on type '%s{resType}' is write-only and may only be updated through PATCH requests to the parent resource (other resource types in collection '%s{collName}' may have a relationship called '%s{relName}' that supports this operation)"
...
let patchRelSelfNotAllowedForAnyResource relName collName supportsPost supportsDelete =
    ...
    |> Error.setTitle "PATCH relationship not supported"
    |> Error.setDetail
        $"Relationship '%s{relName}' does not support PATCH for any resource in collection '%s{collName}'%s{extraMessage}"
...
let postToManyRelSelfNotAllowedPolymorphic relName resType supportsPatch supportsDelete collName =
    ...
    |> Error.setDetail
        $"Relationship '%s{relName}' on type '%s{resType}' does not support adding members using POST%s{extraMessage} (other resource types in collection '%s{collName}' may have a relationship called '%s{relName}' that supports POST)"
...
let postToManyRelSelfNotAllowedForAnyResource relName collName supportsPatch supportsDelete =
    ...
    |> Error.setTitle "POST relationship not supported"
    |> Error.setDetail
        $"Relationship '%s{relName}' does not support adding members using POST for any resource in collection '%s{collName}'%s{extraMessage}"
...
let deleteToManyRelSelfNotAllowedPolymorphic relName resType supportsPatch supportsPost collName =
    ...
    |> Error.setDetail
        $"Relationship '%s{relName}' on type '%s{resType}' does not support removing members using DELETE%s{extraMessage} (other resource types in collection '%s{collName}' may have a relationship called '%s{relName}' that supports DELETE)"
...
let deleteToManyRelSelfNotAllowedForAnyResource relName collName supportsPatch supportsPost =
    ...
    |> Error.setTitle "DELETE relationship not supported"
    |> Error.setDetail
        $"Relationship '%s{relName}' does not support removing members using DELETE for any resource in collection '%s{collName}'%s{extraMessage}"
...
let relModifySelfWhileSkip () =
    ...
    |> Error.setTitle "No relationship value"
    |> Error.setDetail
        "The relationship was updated, but the server has erroneously chosen not to disclose the value of the updated relationship"
...
let customOpNotDefinedPolymorphic linkName resType collName =
    ...
    |> Error.setDetail
        $"Operation '%s{linkName}' is not defined for resource type '%s{resType}' (it may exist for other resource types in collection '%s{collName}')"

let customOpVerbNotDefinedPolymorphic linkName resType invalidVerb allowHeader collName =
    Error.create 405
    |> Error.setTitle "Method not allowed"
    |> Error.setDetail
        $"Operation '%s{linkName}' on type '%s{resType}' does not support %s{invalidVerb} (other resource types in collection '%s{collName}' may have an operation '%s{linkName}' supporting %s{invalidVerb})"
...
let customOpVerbNotDefinedForAnyResource linkName invalidVerb collName allowHeader =
    ...
    |> Error.setTitle "Method not allowed"
    |> Error.setDetail
        $"Operation '%s{linkName}' does not support %s{invalidVerb} for any resource in collection '%s{collName}'"
    |> Error.addHeader "Allow" allowHeader
...
let linkOrRelationshipDoesNotExistForAnyResource linkOrRelName collName =
    ...
    |> Error.setDetail
        $"The link or relationship '%s{linkOrRelName}' does not exist for any resource in collection '%s{collName}'"
