...
module internal FieldTracker =
    ...
    let trackFieldUsage<'ctx>
        ...
        =
        let fieldUsage =
            ConcurrentDictionary<struct (ResourceTypeName * FieldName), FieldUsage>()
        ...
        let addFieldUsage typeName fieldName usageType =
            fieldUsage.AddOrUpdate(
                ...
                fun _ oldUsageType ->
                    ...
                    | FieldUsage.Explicit, _
                    | _, FieldUsage.Explicit -> FieldUsage.Explicit
                    | FieldUsage.Implicit, _
                    | _, FieldUsage.Implicit -> FieldUsage.Implicit
                    | FieldUsage.Excluded, FieldUsage.Excluded -> FieldUsage.Excluded
            )
            |> ignore

        let rec trackUsageForResourceType
            (relNameIfRelationshipSelfAndNotRecursiveCall: FieldName option)
            ...
            let relNamesAndAllowedTypes =
                Array.concat [
                    ...
                    |> Array.map (fun r -> r.Name, r.AllowedTypes)
            ...
            for attrName, requiresExplicitInclude in attrNamesAndRequiresExplicitInclude do
                let usageType =
                    ...
                    | false, _ ->
                        if requiresExplicitInclude then
                            FieldUsage.Excluded
                        else
                            FieldUsage.Implicit
            ...
            for relName, allowedTypes in relNamesAndAllowedTypes do
                ...
                if shouldInclude then
                    ...
                    | Some ts ->
                        ts
                        |> Seq.iter (trackUsageForResourceType None (currentIncludePath @ [ relName ]))

        task {
            match relationshipOperationResourceTypeNameAndFieldName with
            | None -> ()
            | Some(usedTypeName, fieldName) -> addFieldUsage usedTypeName fieldName FieldUsage.Explicit

            match consumedFieldNamesWithType with
            | None -> ()
            | Some(typeName, fieldNames) ->
                for fn in fieldNames do
                    addFieldUsage typeName fn FieldUsage.Explicit

            primaryResourceTypes
            |> List.iter (trackUsageForResourceType relNameIfRelationshipSelf [])
