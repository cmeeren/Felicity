...
module private QueryParseHelpers =
    ...
    let parseInt getData (value: string) =
        ...
        | false, _ ->
            Error [
                invalidParsedErrMsg (FromQuery(getData value)) "The value must be a valid integer"
            ]
    ...
    let parseFloat getData (value: string) =
        ...
        | false, _ ->
            Error [
                invalidParsedErrMsg (FromQuery(getData value)) "The value must be a valid number"
            ]

    /// Converts a string conforming to the ISO 8601-1:2019 format to a DateTime.
    let parseDateTime getData value =
        try
            System.Text.Json.JsonSerializer.Deserialize<DateTime>("\"" + value + "\"") |> Ok
        with _ ->
            Error [
                invalidParsedErrMsg (FromQuery(getData value)) "The value must be a valid ISO 8601-1:2019 date-time"
            ]
    ...
    let parseDateTimeOffsetAllowMissingOffset getData value =
        try
            System.Text.Json.JsonSerializer.Deserialize<DateTimeOffset>("\"" + value + "\"")
            |> Ok
        with _ ->
            Error [
                invalidParsedErrMsg (FromQuery(getData value)) "The value must be a valid ISO 8601-1:2019 date-time"
    ...
    let parseDateTimeOffset =
        ...
        fun getData value ->
            ...
            try
                ...
                if offsetRegex.IsMatch value then
                    Ok res
                else
                    Error [ invalidParsedErrMsg (FromQuery(getData value)) errMsg ]
            with _ ->
                Error [ invalidParsedErrMsg (FromQuery(getData value)) errMsg ]



type ListFilter<'ctx, 'a>
    internal
    (
        fieldName: string,
...
type SingleFilter<'ctx, 'a>
            ...
            member _.Get(ctx, req, _) =
                ...
                | true, str ->
                    if not allowCommas && (str.Contains ",") then
                        ...
                        parse ctx getValueData str |> TaskResult.map Some
        }
    ...
    member _.Bool: SingleFilter<'ctx, bool> =
        SingleFilter<'ctx, bool>(
            fieldName,
            (fun _ getData v -> parseBool getData v |> Task.result),
            ?operator = operator,
            ...
            member _.Get(ctx, req, _) =
                ...
                | true, str ->
                    if str.Contains "," then
                        ...
                        parse ctx getValueData value |> TaskResult.map (fun s -> Some(s, isDescending))
        }
        ...
        { new RequestGetter<'ctx, int option> with
            ...
            member _.Get(ctx, req, _) =
                ...
                | true, value ->
                    ...
                    | false, _ ->
                        Error [
                            invalidParsedErrMsg (FromQuery valueData) "The value must be a valid integer"
                        ]
                        |> Task.result
                    | true, i ->
                        match min, max with
                        | Some min, _ when i < min ->
                            Error [
                                invalidParsedErrMsg
                                    (FromQuery valueData)
                                    $"The value must be greater than or equal to %i{min}"
                            ]
                            |> Task.result
                        | _, Some max when i > max ->
                            Error [
                                invalidParsedErrMsg
                                    (FromQuery valueData)
                                    $"The value must be less than or equal to %i{max}"
                            ]
                            |> Task.result
...
type Filter =
    ...
    static member Field(id: Id<'ctx, 'entity, 'id>) =
        SingleFilter<'ctx, 'id>(
            ...
            fun ctx getData v ->
                id.toDomain ctx v
                |> TaskResult.mapError (List.map (fun f -> f (FromQuery(getData v))))
        )

    static member private Field
        (
            field: FieldQueryParser<'ctx, 'entity, 'attr, 'serialized>,
            toSerialized: (string -> ParsedValueFromQueryData) -> string -> Result<'serialized, Error list>
        ) =
        SingleFilter<'ctx, 'attr>(
            field.Name,
            fun ctx getData v ->
                toSerialized getData v
                |> Task.result
                |> TaskResult.bind (field.ToDomain ctx (string<'serialized> >> getData >> FromQuery))
        )

    static member Field
        (
            field: FieldQueryParser<'ctx, 'entity, 'attr, 'serialized>,
            toSerialized: string -> Result<'serialized, Error list>
        ) =
        SingleFilter<'ctx, 'attr>(
            field.Name,
            fun ctx getData v ->
    ...
    static member Field
        ...
        SingleFilter<'ctx, 'attr>(
            ...
            fun ctx getData v ->
                toSerialized v
                |> Result.requireSome [ invalidParsedNone (getData v |> FromQuery) ]
                |> Task.result
                |> TaskResult.bind (field.ToDomain ctx (string<'serialized> >> getData >> FromQuery))
        )

    static member Field(field: FieldQueryParser<'ctx, 'entity, 'attr, string>) = Filter.Field(field, Ok)

    static member Field(field: FieldQueryParser<'ctx, 'entity, 'attr, bool>) = Filter.Field(field, parseBool)

    static member Field(field: FieldQueryParser<'ctx, 'entity, 'attr, int>) = Filter.Field(field, parseInt)

    static member Field(field: FieldQueryParser<'ctx, 'entity, 'attr, float>) = Filter.Field(field, parseFloat)

    static member Field(field: FieldQueryParser<'ctx, 'entity, 'attr, DateTime>) = Filter.Field(field, parseDateTime)
    ...
    static member private Field
        (
            path: Relationship<'ctx, 'entity, 'relatedEntity, 'relatedId>,
            field: FieldQueryParser<'ctx, 'relatedEntity, 'attr, 'serialized>,
            toSerialized: (string -> ParsedValueFromQueryData) -> string -> Result<'serialized, Error list>
        ) : SingleFilter<'ctx, 'attr> =
        SingleFilter<'ctx, 'attr>(
    ...
    static member Field
        (
            path: Relationship<'ctx, 'entity, 'relatedEntity, 'relatedId>,
            field: FieldQueryParser<'ctx, 'relatedEntity, 'attr, string>
        ) : SingleFilter<'ctx, 'attr> =
    ...
    static member Field
        (
            path: Relationship<'ctx, 'entity, 'relatedEntity, 'relatedId>,
            field: FieldQueryParser<'ctx, 'relatedEntity, 'attr, bool>
        ) : SingleFilter<'ctx, 'attr> =
    ...
    static member Field
        (
            path: Relationship<'ctx, 'entity, 'relatedEntity, 'relatedId>,
            field: FieldQueryParser<'ctx, 'relatedEntity, 'attr, int>
        ) : SingleFilter<'ctx, 'attr> =
    ...
    static member Field
        (
            path: Relationship<'ctx, 'entity, 'relatedEntity, 'relatedId>,
            field: FieldQueryParser<'ctx, 'relatedEntity, 'attr, float>
        ) : SingleFilter<'ctx, 'attr> =
    ...
    static member Field
        (
            path: Relationship<'ctx, 'entity, 'relatedEntity, 'relatedId>,
            field: FieldQueryParser<'ctx, 'relatedEntity, 'attr, DateTime>
        ) : SingleFilter<'ctx, 'attr> =
    ...
    static member Field
        (
            path: Relationship<'ctx, 'entity, 'relatedEntity, 'relatedId>,
            field: FieldQueryParser<'ctx, 'relatedEntity, 'attr, DateTimeOffset>
        ) : SingleFilter<'ctx, 'attr> =
        // Note: We allow missing offset here because it's checked in the attribute, if relevant.
    ...
    static member private Field
        (
            path1: Relationship<'ctx, 'entity, 'relatedEntity1, 'relatedId1>,
            path2: Relationship<'ctx, 'relatedEntity1, 'relatedEntity2, 'relatedId2>,
            field: FieldQueryParser<'ctx, 'relatedEntity2, 'attr, 'serialized>,
            toSerialized: (string -> ParsedValueFromQueryData) -> string -> Result<'serialized, Error list>
        ) : SingleFilter<'ctx, 'attr> =
        SingleFilter<'ctx, 'attr>(
            path1.Name + "." + path2.Name + "." + field.Name,
            fun ctx getData v ->
                toSerialized getData v
                |> Task.result
                |> TaskResult.bind (field.ToDomain ctx (string<'serialized> >> getData >> FromQuery))
    ...
    static member Field
        (
            path1: Relationship<'ctx, 'entity, 'relatedEntity1, 'relatedId1>,
            path2: Relationship<'ctx, 'relatedEntity1, 'relatedEntity2, 'relatedId2>,
            field: FieldQueryParser<'ctx, 'relatedEntity2, 'attr, string>
        ) : SingleFilter<'ctx, 'attr> =
    ...
    static member Field
        (
            path1: Relationship<'ctx, 'entity, 'relatedEntity1, 'relatedId1>,
            path2: Relationship<'ctx, 'relatedEntity1, 'relatedEntity2, 'relatedId2>,
            field: FieldQueryParser<'ctx, 'relatedEntity2, 'attr, bool>
        ) : SingleFilter<'ctx, 'attr> =
    ...
    static member Field
        (
            path1: Relationship<'ctx, 'entity, 'relatedEntity1, 'relatedId1>,
            path2: Relationship<'ctx, 'relatedEntity1, 'relatedEntity2, 'relatedId2>,
            field: FieldQueryParser<'ctx, 'relatedEntity2, 'attr, int>
        ) : SingleFilter<'ctx, 'attr> =
    ...
    static member Field
        (
            path1: Relationship<'ctx, 'entity, 'relatedEntity1, 'relatedId1>,
            path2: Relationship<'ctx, 'relatedEntity1, 'relatedEntity2, 'relatedId2>,
            field: FieldQueryParser<'ctx, 'relatedEntity2, 'attr, float>
        ) : SingleFilter<'ctx, 'attr> =
    ...
    static member Field
        (
            path1: Relationship<'ctx, 'entity, 'relatedEntity1, 'relatedId1>,
            path2: Relationship<'ctx, 'relatedEntity1, 'relatedEntity2, 'relatedId2>,
            field: FieldQueryParser<'ctx, 'relatedEntity2, 'attr, DateTime>
        ) : SingleFilter<'ctx, 'attr> =
    ...
    static member Field
        (
            path1: Relationship<'ctx, 'entity, 'relatedEntity1, 'relatedId1>,
            path2: Relationship<'ctx, 'relatedEntity1, 'relatedEntity2, 'relatedId2>,
            field: FieldQueryParser<'ctx, 'relatedEntity2, 'attr, DateTimeOffset>
        ) : SingleFilter<'ctx, 'attr> =
        // Note: We allow missing offset here because it's checked in the attribute, if relevant.
    ...
    static member private Field
        (
            path1: Relationship<'ctx, 'entity, 'relatedEntity1, 'relatedId1>,
            path2: Relationship<'ctx, 'relatedEntity1, 'relatedEntity2, 'relatedId2>,
            path3: Relationship<'ctx, 'relatedEntity2, 'relatedEntity3, 'relatedId3>,
            field: FieldQueryParser<'ctx, 'relatedEntity3, 'attr, 'serialized>,
            toSerialized: (string -> ParsedValueFromQueryData) -> string -> Result<'serialized, Error list>
        ) : SingleFilter<'ctx, 'attr> =
        SingleFilter<'ctx, 'attr>(
            path1.Name + "." + path2.Name + "." + path3.Name + "." + field.Name,
            fun ctx getData v ->
                toSerialized getData v
                |> Task.result
                |> TaskResult.bind (field.ToDomain ctx (string<'serialized> >> getData >> FromQuery))
    ...
    static member Field
        (
            ...
            path2: Relationship<'ctx, 'relatedEntity1, 'relatedEntity2, 'relatedId2>,
            path3: Relationship<'ctx, 'relatedEntity2, 'relatedEntity3, 'relatedId3>,
            field: FieldQueryParser<'ctx, 'relatedEntity3, 'attr, string>
        ) : SingleFilter<'ctx, 'attr> =
    ...
    static member Field
        (
            ...
            path2: Relationship<'ctx, 'relatedEntity1, 'relatedEntity2, 'relatedId2>,
            path3: Relationship<'ctx, 'relatedEntity2, 'relatedEntity3, 'relatedId3>,
            field: FieldQueryParser<'ctx, 'relatedEntity3, 'attr, bool>
        ) : SingleFilter<'ctx, 'attr> =
    ...
    static member Field
        (
            ...
            path2: Relationship<'ctx, 'relatedEntity1, 'relatedEntity2, 'relatedId2>,
            path3: Relationship<'ctx, 'relatedEntity2, 'relatedEntity3, 'relatedId3>,
            field: FieldQueryParser<'ctx, 'relatedEntity3, 'attr, int>
        ) : SingleFilter<'ctx, 'attr> =
    ...
    static member Field
        (
            ...
            path2: Relationship<'ctx, 'relatedEntity1, 'relatedEntity2, 'relatedId2>,
            path3: Relationship<'ctx, 'relatedEntity2, 'relatedEntity3, 'relatedId3>,
            field: FieldQueryParser<'ctx, 'relatedEntity3, 'attr, float>
        ) : SingleFilter<'ctx, 'attr> =
    ...
    static member Field
        (
            ...
            path2: Relationship<'ctx, 'relatedEntity1, 'relatedEntity2, 'relatedId2>,
            path3: Relationship<'ctx, 'relatedEntity2, 'relatedEntity3, 'relatedId3>,
            field: FieldQueryParser<'ctx, 'relatedEntity3, 'attr, DateTime>
        ) : SingleFilter<'ctx, 'attr> =
    ...
    static member Field
        (
            ...
            path2: Relationship<'ctx, 'relatedEntity1, 'relatedEntity2, 'relatedId2>,
            path3: Relationship<'ctx, 'relatedEntity2, 'relatedEntity3, 'relatedId3>,
            field: FieldQueryParser<'ctx, 'relatedEntity3, 'attr, DateTimeOffset>
        ) : SingleFilter<'ctx, 'attr> =
        // Note: We allow missing offset here because it's checked in the attribute, if relevant.
    ...
    static member DateTimeOffsetAllowMissingOffset(name) : SingleFilter<'ctx, DateTimeOffset> =
        SingleFilter<'ctx, DateTimeOffset>(
            name,
            fun _ getData v -> parseDateTimeOffsetAllowMissingOffset getData v |> Task.result
        )
...
module FilterExtensions =
    ...
    type Filter with
        ...
        static member Field
            (
                field: FieldQueryParser<'ctx, 'entity, 'attr, 'serialized>,
                toSerialized: string -> Result<'serialized, string>
            ) =
            SingleFilter<'ctx, 'attr>(
                field.Name,
                fun ctx getData v ->
                    toSerialized v
                    |> Result.mapError (invalidParsedErrMsg (getData v |> FromQuery) >> List.singleton)
                    |> Task.result
                    |> TaskResult.bind (field.ToDomain ctx (string<'serialized> >> getData >> FromQuery))
            )
        ...
        static member Field
            (
                field: FieldQueryParser<'ctx, 'entity, 'attr, 'serialized>,
                toSerialized: string -> Result<'serialized, string list>
            ) =
            SingleFilter<'ctx, 'attr>(
                field.Name,
                fun ctx getData v ->
                    toSerialized v
                    |> Result.mapError (List.map (invalidParsedErrMsg (getData v |> FromQuery)))
                    |> Task.result
                    |> TaskResult.bind (field.ToDomain ctx (string<'serialized> >> getData >> FromQuery))
            )
...
type Sort =
    ...
    static member private ParsedTaskRes'
        (parse: 'ctx -> (string -> ParsedValueFromQueryData) -> string -> Task<Result<'a, Error list>>)
        : SingleSort<'ctx, 'a> =
    ...
    static member ParsedAsync(parse: string -> Async<'a>) : SingleSort<'ctx, 'a> = Sort.ParsedTask(Task.liftAsync parse)
    ...
    static member ParsedOpt(parse: string -> 'a option) : SingleSort<'ctx, 'a> = Sort.ParsedTaskOpt(Task.lift parse)
...
type Query =
    ...
    static member DateTimeOffset(queryParamName) : CustomQueryParam<'ctx, DateTimeOffset> =
        CustomQueryParam<'ctx, DateTimeOffset>(
            queryParamName,
            fun _ getData v -> parseDateTimeOffset getData v |> Task.result
        )
...
type Header =
    ...
    static member ParsedTaskRes(headerName, parse: 'ctx -> string -> Task<Result<'a, string>>) : Header<'ctx, 'a> =
        Header<'ctx, 'a>(
            ...
            fun ctx getData v ->
                parse ctx v
                |> TaskResult.mapError (invalidParsedErrMsg (FromHeader(getData v)) >> List.singleton)
        )
    ...
    static member ParsedTaskRes(headerName, parse: 'ctx -> string -> Task<Result<'a, string list>>) : Header<'ctx, 'a> =
        Header<'ctx, 'a>(
            ...
            fun ctx getData v ->
                parse ctx v
                |> TaskResult.mapError (List.map (invalidParsedErrMsg (FromHeader(getData v))))
        )
    ...
    static member ParsedTaskOpt(headerName, parse: 'ctx -> string -> Task<'a option>) : Header<'ctx, 'a> =
        Header<'ctx, 'a>(
            ...
            fun ctx getData v ->
                parse ctx v
                |> Task.map (Result.requireSome [ invalidParsedNone (FromHeader(getData v)) ])
        )
