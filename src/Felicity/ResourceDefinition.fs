...
module internal LockSpecification =
        ...
        task {
            ...
            | CustomForModification lockSpec, Some resId ->
                if not state.TimedOut then
                    ...
                    | Some(Some lock) -> state.AddLock lock
                    | Some None -> state.SetTimedOut()
                    | None -> ()
    ...
    let lockAll httpCtx ctx req (totalTimeout: TimeSpan option) resId lockSpecs = task {
        let state = new MultiLockState()

        let timeoutTask =
            totalTimeout
            |> Option.map (fun ts -> task {
                do! Task.Delay ts
                state.SetTimedOut()
            })
...
type ResourceDefinition<'ctx, 'entity, 'id> =
    internal
        {
            name: string
    ...
    interface ResourceDefinition<'ctx> with
        ...
        member this.GetIdBoxed entity =
            unbox entity |> this.id.getId |> this.id.fromDomain

        member this.ParseIdBoxed ctx rawId =
            this.id.toDomain ctx rawId |> TaskResult.map box
    ...
    member this.CollectionName(collectionName) =
        { this with
            collectionName = Some collectionName
        }
    ...
    member private this.CreateOtherForResourceCreationLockSpec
        (
            resDef: ResourceDefinition<'ctx, 'otherEntity, 'otherId>,
            ?relationshipForPostOperation: OptionalRequestGetter<'ctx, 'otherId>,
            ?nullableRelationshipForPostOperation: OptionalRequestGetter<'ctx, 'otherId option>
        ) =
        OtherForResourceCreation
            {
                    ...
                    fun ctx req -> task {
                        ...
                        | None ->
                            ...
                            | Some rel ->
                                ...
                                | Error _ -> return None
                                | Ok otherId -> return otherId |> Option.bind id |> Option.map resDef.id.fromDomain
                        | Some rel ->
                            match! rel.Get(ctx, req, None) with
                            | Error _ -> return None
                ...
                OtherLockSpecs =
                    ...
                    | [] ->
                        failwith
                            $"Resource type '%s{this.name}' can not lock other resource '%s{resDef.name}' because the other resource does not have a lock specification"
                    | xs -> xs
            }

    member private this.CreateOtherForModificationLockSpec
        (
            resDef: ResourceDefinition<'ctx, 'otherEntity, 'otherId>,
            getOtherIdForResourceOperation: 'ctx -> 'id -> Task<'otherId option>
        ) =
    ...
    member this.Lock(?timeout) =
        ...
        { this with
            lockSpecs = this.lockSpecs @ [ this.CreateFelicityLockSpec(?timeout = timeout) ]
        }
    ...
    member this.CustomLock(getLock: 'ctx -> 'id -> Task<IDisposable option>) =
        { this with
            lockSpecs = this.lockSpecs @ [ this.CreateCustomLockSpec(fun ctx id -> getLock ctx id) ]
        }
    ...
    /// the lock times out, or Some with an IDisposable that releases the lock when
    /// disposed.
    member this.CustomLock(getLock: 'id -> Task<IDisposable option>) = this.CustomLock(fun _ id -> getLock id)
    ...
    member this.CustomResourceCreationLock(getLock: 'ctx -> Task<IDisposable option>) =
        { this with
            lockSpecs =
                this.lockSpecs
                @ [ this.CreateCustomResourceCreationLockSpec(fun ctx -> getLock ctx) ]
        }
    ...
    member this.LockOtherForModification
        (
            resDef: ResourceDefinition<'ctx, 'otherEntity, 'otherId>,
            getOtherIdForResourceOperation: 'id -> Task<'otherId option>
        ) =
    ...
    member this.LockOtherForModification
        (
            resDef: ResourceDefinition<'ctx, 'otherEntity, 'otherId>,
            getOtherIdForResourceOperation: 'id -> Async<'otherId option>
        ) =
        this.LockOtherForModification(
            resDef,
            getOtherIdForResourceOperation = Task.liftAsync2 (fun _ -> getOtherIdForResourceOperation)
        )
    ...
    member this.LockOtherForModification
        (
            resDef: ResourceDefinition<'ctx, 'otherEntity, 'otherId>,
            getOtherIdForResourceOperation: 'ctx -> 'id -> Async<'otherId option>
        ) =
        this.LockOtherForModification(
            resDef,
            getOtherIdForResourceOperation = Task.liftAsync2 getOtherIdForResourceOperation
        )
    ...
    member this.LockOtherForModification
        (
            resDef: ResourceDefinition<'ctx, 'otherEntity, 'otherId>,
            getOtherIdForResourceOperation: 'ctx -> 'id -> 'otherId option
        ) =
        this.LockOtherForModification(
            resDef,
            getOtherIdForResourceOperation = Task.lift2 getOtherIdForResourceOperation
        )
    ...
    member this.LockOtherForModification
        (
            resDef: ResourceDefinition<'ctx, 'otherEntity, 'otherId>,
            getOtherIdForResourceOperation: 'id -> 'otherId option
        ) =
    ...
    member this.LockOtherForModification
        (
            resDef: ResourceDefinition<'ctx, 'otherEntity, 'otherId>,
            getOtherIdForResourceOperation: 'ctx -> 'id -> 'otherId
        ) =
    ...
    /// other resource.
    member this.LockOtherForResourceCreation
        (
            resDef: ResourceDefinition<'ctx, 'otherEntity, 'otherId>,
            relationshipForPostOperation: OptionalRequestGetter<'ctx, 'otherId>
        ) =
        { this with
            lockSpecs =
                this.lockSpecs
                @ [
                    this.CreateOtherForResourceCreationLockSpec(
                        resDef,
    ...
    member this.LockOtherForResourceCreation
        (
            resDef: ResourceDefinition<'ctx, 'otherEntity, 'otherId>,
            relationshipForPostOperation: OptionalRequestGetter<'ctx, 'otherId option>
        ) =
        { this with
            lockSpecs =
                this.lockSpecs
                @ [
                    this.CreateOtherForResourceCreationLockSpec(
                        resDef,
                        nullableRelationshipForPostOperation = relationshipForPostOperation
                    )
                ]
        }
    ...
    member this.MultiLockTotalTimeout(timeout: TimeSpan) =
        if this.lockSpecs.Length < 2 then
            failwith
                $"At least two locks must be present in order to call MultiLockTotalTimeout for resource '%s{this.name}'"

        { this with
            lockTotalTimeout = Some timeout
        }
