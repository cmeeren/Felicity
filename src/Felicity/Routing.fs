...
let internal jsonApiEndpoints
    relativeRootWithLeadingSlash
    (getCtx: HttpContext -> Task<Result<'ctx, Error list>>)
    collections
    : Endpoint list =
    ...
    let validateRequestWithOverrides (cfg: RequestValidationConfig) : HttpHandler =
        fun next httpCtx ->
            ...
            let errs = [
                if cfg.ValidateAccept && (not <| RequestValidation.acceptsJsonApi httpCtx) then
                    invalidAccept ()
                if cfg.ValidateContentType && (RequestValidation.hasNonJsonApiContent httpCtx) then
                    invalidContentType ()
                if cfg.ValidateAccept && (RequestValidation.allJsonApiAcceptsHaveParams httpCtx) then
                    invalidAcceptParams ()
                if
                    cfg.ValidateContentType
                    && (RequestValidation.jsonApiContentTypeHasParams httpCtx)
                then
            ...
            subRoute relativeRootWithLeadingSlash [
                subRoute ("/" + collName) [
                    ...
                    route
                        ...
                        (let allowHeader =
                            [
                                if ops.getCollection.IsSome then
                                    "GET"
                                    "HEAD"
                                if ops.postCollection.IsSome then
                                    "POST"
                            ]
                            |> String.concat ", "
                    ...
                    | Some getById ->
                        ...
                        let getLinkAllowHeader (link: LinkOperations<'ctx>) =
                            [
                                if link.get.IsSome then
                                    "GET"
                                    "HEAD"
                                if link.post.IsSome then
                                    "POST"
                                if link.patch.IsSome then
                                    "PATCH"
                                if link.delete.IsSome then
                                    "DELETE"
                            ]
                            |> String.concat ", "
