  ...
  member _.WithEntity(resourceDef: ResourceDefinition<'ctx, 'entity, 'id>, entity: 'entity) : HttpHandler =
    fun next httpCtx ->
      task {
        ...
        let primaryResourceTypes = primaryResourceTypesForFieldTracking |> Option.defaultValue [resourceDef.name]
        let! fieldTrackerHandler = httpCtx.RequestServices.GetRequiredService<FieldTracker<'ctx>>().TrackFields(primaryResourceTypes, ctx, req)
  ...
  member _.WithPolymorphicEntities(polyBuilders: PolymorphicBuilder<'ctx> list) : HttpHandler =
    fun next httpCtx ->
      task {
        ...
        let primaryResourceTypes = primaryResourceTypesForFieldTracking |> Option.defaultValue (polyBuilders |> List.map (fun b -> b.resourceDef.TypeName) |> List.distinct)
        let! fieldTrackerHandler = httpCtx.RequestServices.GetRequiredService<FieldTracker<'ctx>>().TrackFields(primaryResourceTypes, ctx, req)
        return! (fieldTrackerHandler >=> jsonApiWithETag<'ctx> doc) next httpCtx
      }
