...
module private RelationshipHelpers =
    ...
    let getIncludedForSelfUrl
        httpCtx
        ctx
        (req: Request)
        (resp: ResponseBuilder<'ctx>)
        relName
        parentResDef
        parentEntity
        =
        ...
        let reqForIncluded =
            { req with
                Includes = req.Includes |> List.filter (List.tryHead >> (=) (Some relName))
            }
...
type ToOneRelationshipRelatedGetter<'ctx, 'entity, 'relatedEntity, 'relatedId> =
    internal
        {
            name: string
...
type ToOneRelationshipIncludedGetter<'ctx, 'relatedEntity> =
    internal
        {
            name: string
            ...
            member _.Get(ctx, req, includedTypeAndId) =
                ...
                | Ok(Some(rels, relsPointer)) ->
                    ...
                    | true, (:? ToOne as rel) ->
                        match rel with
                        | { data = Skip } ->
                            Error [ relMissingData this.name (relsPointer + "/" + this.name) ]
                            |> Task.result
                        | { data = Include identifier } when
                            not (this.allowedTypes |> List.contains identifier.``type``)
                            ->
                            let pointer = relsPointer + "/" + this.name + "/data/type"

                            Error [ relInvalidType this.name identifier.``type`` this.allowedTypes pointer ]
                            |> Task.result
...
type ToOneRelationship<'ctx, 'setCtx, 'entity, 'relatedEntity, 'relatedId> =
    internal
        {
            name: string
            setOrder: int
            ...
            idParsers:
                Map<ResourceTypeName, 'ctx -> ResourceId -> Task<Result<'relatedId, (ParsedValueInfo -> Error) list>>> option
            get: ('ctx -> 'entity -> Task<'relatedEntity Skippable>) option
            set:
                ('ctx
                    -> 'setCtx
                    -> Pointer
                    -> 'relatedId * ResourceIdentifier
                    -> 'entity
                    ...
                    | true, (:? ToOne as rel) ->
                        ...
                        | { data = Include identifier } ->
                            ...
                            | false, _ ->
                                ...
                                Error [ relInvalidType this.name identifier.``type`` allowedTypes pointer ]
                                |> Task.result
                            | true, parseId ->
    ...
    interface ProhibitedRequestGetter with
        ...
        member this.GetErrors(req, includedTypeAndId) =
            ...
            | Error errs -> errs
            | Ok(Some {
                          data = Some { relationships = Include rels }
                      }) when rels.ContainsKey this.name ->
        ...
        member this.Set ctx req entity numSetters = task {
            ...
            | Ok(Some {
                ...
                | Some _, (true, rel) ->
                    return
                        failwith
                            $"Framework bug: Expected relationship '%s{this.name}' to be deserialized to %s{typeof<ToOne>.FullName}, but was %s{rel.GetType().FullName}"
    ...
    member this.Included(getParser: RequestParserHelper<'ctx> -> RequestParser<'ctx, 'relatedEntity>) =
        ToOneRelationshipIncludedGetter.Create(
            this.name,
            getParser,
            this.idParsers
    ...
    interface FieldQueryParser<'ctx, 'entity, 'relatedId, string> with
        ...
        member this.ToDomain ctx getInfo str =
            ...
            | [] ->
                failwith
                    $"Relationship '%s{this.name}' does not contain any ID parsers and may not be used to parse query IDs"
            | [ _, parseId ] ->
                parseId ctx str
                |> TaskResult.mapError (List.map (fun getErr -> getErr (getInfo str)))
            | _ :: _ :: _ ->
                failwith
                    $"Relationship '%s{this.name}' contains ID parsers for several types and may therefore not be used to parse query IDs"
    ...
    interface RelationshipHandlers<'ctx> with
        ...
        member this.PatchSelf =
            (this.get, this.set)
            ||> Option.map2 (fun getRelated set ->
                let idParsers =
                    this.idParsers
                    |> Option.defaultWith (fun () ->
                        failwithf
                    ...
                    fun next httpCtx -> task {
                        ...
                        | Ok() ->
                            ...
                            | Ok setCtx ->
                                ...
                                | Ok(Some { data = None }) ->
                                    return!
                                        handleErrors [ relInvalidNull parentTypeName this.name "/data" ] next httpCtx
                                    ...
                                    | Ok() ->
                                        ...
                                        | Ok entity1 ->
                                            ...
                                            | false, _ ->
                                                ...
                                                return!
                                                    handleErrors
                                                        [ relInvalidTypeSelf id.``type`` allowedTypes "/data/type" ]
                                                        next
                                                        httpCtx
                                            | true, parseId ->
                                                let! entity2Res =
    ...
    member this.GetAsync(get: Func<'ctx, 'entity, Async<'relatedEntity>>) = this.GetTask(Task.liftAsyncFunc2 get)

    member this.GetAsync(get: Func<'entity, Async<'relatedEntity>>) = this.GetTask(Task.liftAsyncFunc get)
    ...
    member private this.SetTaskRes
        (set: Func<'ctx, 'setCtx, Pointer, 'relatedId * ResourceIdentifier, 'entity, Task<Result<'entity, Error list>>>)
        =
        if this.idParsers.IsNone then
            failwithf "Can only add setter if the polymorphic resource definition contains ID parsers."

        { this with
            set = Some(fun ctx setCtx ptr relIdWithIdentifier e -> set.Invoke(ctx, setCtx, ptr, relIdWithIdentifier, e))
        }
    ...
    member this.SetTaskRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'setCtx, 'lookupType, 'entity, Task<Result<'entity, Error list>>>
        ) =
    ...
    member this.SetTaskRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'lookupType, 'entity, Task<Result<'entity, Error list>>>
        ) =
    ...
    member this.SetAsyncRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'setCtx, 'lookupType, 'entity, Async<Result<'entity, Error list>>>
        ) =
    ...
    member this.SetAsyncRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'lookupType, 'entity, Async<Result<'entity, Error list>>>
        ) =
    ...
    member this.SetTask
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'setCtx, 'lookupType, 'entity, Task<'entity>>
        ) =
    ...
    member this.SetAsync(set: Func<'relatedId, 'entity, Async<'entity>>) = this.SetTask(Task.liftAsyncFunc2 set)

    member this.SetAsync
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'setCtx, 'lookupType, 'entity, Async<'entity>>
        ) =
    ...
    member this.SetRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'setCtx, 'lookupType, 'entity, Result<'entity, Error list>>
        ) =
    ...
    member this.SetRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'lookupType, 'entity, Result<'entity, Error list>>
        ) =
    ...
    member this.BeforeModifySelfTaskRes(f: Func<'setCtx, 'entity, Task<Result<'entity, Error list>>>) =
        { this with
            beforeModifySelf = (fun ctx e -> f.Invoke(ctx, e))
        }
    ...
    member this.AfterModifySelfTaskRes(f: 'setCtx -> 'entity -> 'entity -> Task<Result<unit, Error list>>) =
        { this with
            afterModifySelf = Some(fun ctx eOld eNew -> f ctx eOld eNew |> TaskResult.map (fun () -> eNew))
        }

    member this.AfterModifySelfTaskRes(f: 'setCtx -> 'entity -> Task<Result<'entity, Error list>>) =
        { this with
            afterModifySelf = Some(fun ctx _ e -> f ctx e)
        }
    ...
    member this.ModifyGetSelfResponse(getHandler: 'ctx -> 'entity -> 'relatedEntity -> HttpHandler) =
        { this with
            modifyGetSelfResponse = getHandler
        }
    ...
    member this.ModifyPatchSelfOkResponse(getHandler: 'setCtx -> 'entity -> 'relatedEntity -> HttpHandler) =
        { this with
            modifyPatchSelfOkResponse = getHandler
        }
    ...
    member this.ModifyPatchSelfAcceptedResponse(getHandler: 'setCtx -> 'entity -> HttpHandler) =
        { this with
            modifyPatchSelfAcceptedResponse = getHandler
        }
...
type ToOneNullableRelationshipRelatedGetter<'ctx, 'entity, 'relatedEntity, 'relatedId> =
    internal
        {
            name: string
    ...
    static member internal Create
        (
            ...
            idGetter: RequestGetter<'ctx, ('relatedId * ResourceIdentifier) option option>,
            getRelated: ResourceLookup<'ctx, 'relatedEntity, 'relatedId>
        ) : ToOneNullableRelationshipRelatedGetter<'ctx, 'entity, 'relatedEntity, 'relatedId> =
        {
            name = name
            idGetter = idGetter
            getRelated = getRelated
                ...
                |> TaskResult.bind ( // ID did not fail parsing, but may be missing or null
                    Option.traverseTaskResult ( // ID was present, but may be null
                        Option.traverseTaskResult (fun (resId, identifier) -> // ID was not null
                            this.getRelated.GetById ctx resId
                            |> TaskResult.mapError (
                                List.map (Error.setSourcePointer ("/data/relationships/" + this.name + "/data"))
                            )
                            |> TaskResult.requireSome [
                                relatedResourceNotFound
                                    identifier.``type``
    ...
    member this.AsNonNullable =
        let nonNullIdGetter =
            { new RequestGetter<'ctx, ('relatedId * ResourceIdentifier) option> with
                ...
                member _.Get(ctx, req, includedTypeAndId) = task {
                    ...
                    | Ok(Some None) ->
                        return
                            Error [
                                setRelNullNotAllowed this.name
                                |> Error.setSourcePointer ("/data/relationships/" + this.name + "/data")
                            ]
                        ...
                        | { data = Include identifier } ->
                            ...
                            |> Option.traverseTaskResult (fun id ->
                                if not (this.allowedTypes |> List.contains id.``type``) then
                                    ...
                                    Error [ relInvalidType this.name id.``type`` this.allowedTypes pointer ]
                                    |> Task.result
                                else
...
type ToOneNullableRelationship<'ctx, 'setCtx, 'entity, 'relatedEntity, 'relatedId> =
    internal
        {
            name: string
            setOrder: int
        ...
        member this.Set ctx req entity numSetters = task {
            ...
            | Ok(Some {
                ...
                | Some _, (true, rel) ->
                    return
                        failwith
                            $"Framework bug: Expected relationship '%s{this.name}' to be deserialized to %s{typeof<ToOneNullable>.FullName}, but was %s{rel.GetType().FullName}"
            | _ -> return Ok(entity, false) // no relationships provided
        }
    ...
    member this.Included(getParser: RequestParserHelper<'ctx> -> RequestParser<'ctx, 'relatedEntity>) =
        ToOneNullableRelationshipIncludedGetter.Create(
            this.name,
            getParser,
            this.idParsers
            |> Option.map (Map.toList >> List.map fst)
                                                ...
                                                | Ok entity3 ->
                                                    if this.patchSelfReturn202Accepted then
                                                        ...
                                                        | Skip ->
                                                            ...
                                                            return!
                                                                handleErrors [ relModifySelfWhileSkip () ] next httpCtx
                                                        | Include relatedEntity ->
                                                            let! included =
                                                                getIncludedForSelfUrl
                                                                    httpCtx
    ...
    member this.GetAsync(get: Func<'ctx, 'entity, Async<'relatedEntity option>>) = this.GetTask(Task.liftAsyncFunc2 get)

    member this.GetAsync(get: Func<'entity, Async<'relatedEntity option>>) = this.GetTask(Task.liftAsyncFunc get)
    ...
    member private this.SetTaskRes
        ...
        { this with
            set = Some(fun ctx setCtx ptr relId e -> set.Invoke(ctx, setCtx, ptr, relId, e))
        }
    ...
    member this.SetTaskRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'setCtx, 'lookupType option, 'entity, Task<Result<'entity, Error list>>>
        ) =
        this.SetTaskRes(this.toIdSetter getRelated (fun ctx relId e -> set.Invoke(ctx, relId, e)))
    ...
    member this.SetTaskRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'lookupType option, 'entity, Task<Result<'entity, Error list>>>
        ) =
        this.SetTaskRes(this.toIdSetter getRelated (fun _ id e -> set.Invoke(id, e)))
    ...
    member this.SetAsyncRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'setCtx, 'lookupType option, 'entity, Async<Result<'entity, Error list>>>
        ) =
        this.SetTaskRes(getRelated, Task.liftAsyncFunc3 set)
    ...
    member this.SetAsyncRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'lookupType option, 'entity, Async<Result<'entity, Error list>>>
        ) =
        this.SetTaskRes(getRelated, Task.liftAsyncFunc2 set)
    ...
    member this.SetTask
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'setCtx, 'lookupType option, 'entity, Task<'entity>>
        ) =
        this.SetTaskRes(getRelated, (fun ctx related entity -> set.Invoke(ctx, related, entity) |> Task.map Ok))
    ...
    member this.SetTask
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'lookupType option, 'entity, Task<'entity>>
        ) =
        this.SetTaskRes(getRelated, (fun _ related entity -> set.Invoke(related, entity) |> Task.map Ok))
    ...
    member this.SetAsync(set: Func<'relatedId option, 'entity, Async<'entity>>) = this.SetTask(Task.liftAsyncFunc2 set)

    member this.SetAsync
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'setCtx, 'lookupType option, 'entity, Async<'entity>>
        ) =
        this.SetTask(getRelated, Task.liftAsyncFunc3 set)
    ...
    member this.SetAsync
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'lookupType option, 'entity, Async<'entity>>
        ) =
        this.SetTask(getRelated, Task.liftAsyncFunc2 set)
    ...
    member this.SetRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'setCtx, 'lookupType option, 'entity, Result<'entity, Error list>>
        ) =
        this.SetTaskRes(getRelated, Task.liftFunc3 set)
    ...
    member this.SetRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'lookupType option, 'entity, Result<'entity, Error list>>
        ) =
        this.SetTaskRes(getRelated, Task.liftFunc2 set)
    ...
    member this.Set
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'setCtx, 'lookupType option, 'entity, 'entity>
        ) =
        this.SetTaskRes(getRelated, TaskResult.liftFunc3 set)
    ...
    member this.SetNonNullAsyncRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            set: Func<'setCtx, 'lookupType, 'entity, Async<Result<'entity, Error list>>>
        ) =
    ...
    member this.AfterModifySelfTaskRes(f: 'entity -> 'entity -> Task<Result<'entity, Error list>>) =
        { this with
            afterModifySelf = Some(fun _ eOld eNew -> f eOld eNew)
        }

    member this.AfterModifySelfTaskRes(f: 'entity -> 'entity -> Task<Result<unit, Error list>>) =
        { this with
            afterModifySelf = Some(fun _ eOld eNew -> f eOld eNew |> TaskResult.map (fun () -> eNew))
        }
...
type ToManyRelationshipRelatedGetter<'ctx, 'entity, 'relatedEntity, 'relatedId> =
    internal
        {
            name: string
    ...
    static member internal Create
        (
            ...
            idGetter: RequestGetter<'ctx, ('relatedId * ResourceIdentifier) list option>,
            getRelated: ResourceLookup<'ctx, 'relatedEntity, 'relatedId>
        ) : ToManyRelationshipRelatedGetter<'ctx, 'entity, 'relatedEntity, 'relatedId> =
        {
            name = name
            idGetter = idGetter
            getRelated = getRelated
                ...
                |> TaskResult.bind (
                    Option.traverseTaskResult (
                        ...
                        >> List.traverseTaskResultA (fun (i, (resId, identifier)) ->
                            ...
                            |> TaskResult.mapError (
                                List.map (
                                    Error.setSourcePointer ("/data/relationships/" + this.name + "/data/" + string i)
                                )
                            )
                            |> TaskResult.requireSome [
                                relatedResourceNotFound
...
type ToManyRelationshipIncludedGetter<'ctx, 'relatedEntity> =
    internal
        {
            name: string
            getParser: RequestParserHelper<'ctx> -> RequestParser<'ctx, 'relatedEntity>
...
type ToManyRelationship<'ctx, 'setCtx, 'entity, 'relatedEntity, 'relatedId> =
    internal
        {
            name: string
            setOrder: int
    ...
    member private _.toIdSetter
        (getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>)
        (entitySetter: 'setCtx -> 'lookupType list -> 'entity -> Task<Result<'entity, Error list>>)
        =
            ...
            | Ok(Some {
                ...
                | None, (true, _) ->
                    if numSetters[this.name] > 1 then
                        // Provided in request and no setter, but there exists another setter, so ignore
                        return Ok(entity, false)
                    else if this.add.IsNone && this.remove.IsNone then
                        return Error [ setRelReadOnly this.name ("/data/relationships/" + this.name) ]
                    else
                        return
                            Error [
                                setToManyRelReplacementNotSupported
                                    this.name
                                    t
                                    ("/data/relationships/" + this.name)
                                    this.add.IsSome
                                    this.remove.IsSome
                            ]
                ...
                | Some _, (true, rel) ->
                    return
                        failwith
                            $"Framework bug: Expected relationship '%s{this.name}' to be deserialized to %s{typeof<ToMany>.FullName}, but was %s{rel.GetType().FullName}"
            | _ -> return Ok(entity, false) // no relationships provided
        }
    ...
    member this.Included(getParser: RequestParserHelper<'ctx> -> RequestParser<'ctx, 'relatedEntity>) =
        ToManyRelationshipIncludedGetter.Create(
            this.name,
            getParser,
            this.idParsers
            |> Option.map (Map.toList >> List.map fst)
                                        ...
                                        let! entity2Res =
                                            ...
                                            |> Array.traverseTaskResultAIndexed (fun i id ->
                                                ...
                                                | false, _ ->
                                                    ...
                                                    Error [ relInvalidTypeSelf id.``type`` allowedTypes pointer ]
                                                    |> Task.result
                                                | true, parseId ->
                                                    parseId ctx id.id
                                                    |> TaskResult.map (fun x -> x, id)
    ...
    interface RelationshipHandlers<'ctx> with
        ...
        member this.IsSettableWithoutPersist =
            (this.setAll.IsSome || this.add.IsSome || this.remove.IsSome)
            && this.afterModifySelf.IsNone
                            ...
                            | Ok() ->
                                ...
                                | Include relatedEntities ->
                                    ...
                                    let! fieldTrackerHandler =
                                        match this.idParsers with
                                        | None ->
                                            logFieldTrackerPolymorphicRelTraversalWarning
                                                httpCtx
                                                resDef.TypeName
                                                this.name
                                ...
                                let data =
                                    ...
                                    if data.Length = distinct.Length then
                                        ...
                                        for d in duplicates do
                                            logger.LogWarning(
                                                "Resource identifier with type '{ResourceType}' and ID '{ResourceId}' appeared multiple times in the primary data. This is not allowed. Only the first occurrence was used.",
                                                d.``type``,
                                                d.id
                                            )
    ...
    member this.GetAsync(get: Func<'ctx, 'entity, Async<'relatedEntity list>>) = this.GetTask(Task.liftAsyncFunc2 get)

    member this.GetAsync(get: Func<'entity, Async<'relatedEntity list>>) = this.GetTask(Task.liftAsyncFunc get)

    member this.GetSkip(get: Func<'ctx, 'entity, Skippable<'relatedEntity list>>) = this.GetTaskSkip(Task.liftFunc2 get)

    member this.GetSkip(get: Func<'entity, Skippable<'relatedEntity list>>) = this.GetTaskSkip(Task.liftFunc get)

    member this.Get(get: Func<'ctx, 'entity, 'relatedEntity list>) = this.GetTask(Task.liftFunc2 get)

    member this.Get(get: Func<'entity, 'relatedEntity list>) = this.GetTask(Task.liftFunc get)
    ...
    member private this.SetAllTaskRes
        ...
        { this with
            setAll = Some(fun ctx setCtx ptr relIds e -> setAll.Invoke(ctx, setCtx, ptr, relIds, e))
        }
    ...
    member this.SetAllTaskRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            setAll: Func<'setCtx, 'lookupType list, 'entity, Task<Result<'entity, Error list>>>
        ) =
    ...
    member this.SetAllTaskRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            setAll: Func<'lookupType list, 'entity, Task<Result<'entity, Error list>>>
        ) =
    ...
    member this.SetAllAsyncRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            setAll: Func<'setCtx, 'lookupType list, 'entity, Async<Result<'entity, Error list>>>
        ) =
    ...
    member this.SetAllAsyncRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            setAll: Func<'lookupType list, 'entity, Async<Result<'entity, Error list>>>
        ) =
    ...
    member this.SetAllTask
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            setAll: Func<'setCtx, 'lookupType list, 'entity, Task<'entity>>
        ) =
    ...
    member this.SetAllTask
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            setAll: Func<'lookupType list, 'entity, Task<'entity>>
        ) =
    ...
    member this.SetAllAsync
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            setAll: Func<'setCtx, 'lookupType list, 'entity, Async<'entity>>
        ) =
    ...
    member this.SetAllAsync
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            setAll: Func<'lookupType list, 'entity, Async<'entity>>
        ) =
    ...
    member this.SetAllRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            setAll: Func<'setCtx, 'lookupType list, 'entity, Result<'entity, Error list>>
        ) =
    ...
    member this.SetAllRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            setAll: Func<'lookupType list, 'entity, Result<'entity, Error list>>
        ) =
    ...
    member this.SetAll
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            setAll: Func<'setCtx, 'lookupType list, 'entity, 'entity>
        ) =
    ...
    member private this.AddTaskRes
        ...
        { this with
            add = Some(fun ctx setCtx ptr relIds e -> add.Invoke(ctx, setCtx, ptr, relIds, e))
        }
    ...
    member this.AddTaskRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            add: Func<'setCtx, 'lookupType list, 'entity, Task<Result<'entity, Error list>>>
        ) =
    ...
    member this.AddTaskRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            add: Func<'lookupType list, 'entity, Task<Result<'entity, Error list>>>
        ) =
    ...
    member this.AddAsyncRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            add: Func<'setCtx, 'lookupType list, 'entity, Async<Result<'entity, Error list>>>
        ) =
    ...
    member this.AddAsyncRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            add: Func<'lookupType list, 'entity, Async<Result<'entity, Error list>>>
        ) =
    ...
    member this.AddTask
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            add: Func<'setCtx, 'lookupType list, 'entity, Task<'entity>>
        ) =
    ...
    member this.AddAsync(add: Func<'relatedId list, 'entity, Async<'entity>>) = this.AddTask(Task.liftAsyncFunc2 add)

    member this.AddAsync
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            add: Func<'setCtx, 'lookupType list, 'entity, Async<'entity>>
        ) =
    ...
    member this.AddRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            add: Func<'setCtx, 'lookupType list, 'entity, Result<'entity, Error list>>
        ) =
    ...
    member this.AddRes
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            add: Func<'lookupType list, 'entity, Result<'entity, Error list>>
        ) =
    ...
    member this.Add
        (
            getRelated: ResourceLookup<'ctx, 'lookupType, 'relatedId>,
            add: Func<'setCtx, 'lookupType list, 'entity, 'entity>
        ) =
    ...
    member private this.RemoveTaskRes
        ...
        { this with
            remove = Some(fun ctx setCtx ptr relIds e -> remove.Invoke(ctx, setCtx, ptr, relIds, e))
        }
    ...
    member this.ModifyPostSelfOkResponse(getHandler: 'setCtx -> 'entity -> 'relatedEntity list -> HttpHandler) =
        { this with
            modifyPostSelfOkResponse = getHandler
        }
    ...
    member this.ModifyPostSelfAcceptedResponse(getHandler: 'setCtx -> 'entity -> HttpHandler) =
        { this with
            modifyPostSelfAcceptedResponse = getHandler
        }
    ...
    member this.ModifyDeleteSelfOkResponse(getHandler: 'setCtx -> 'entity -> 'relatedEntity list -> HttpHandler) =
        { this with
            modifyDeleteSelfOkResponse = getHandler
        }
    ...
    member this.ModifyDeleteSelfAcceptedResponse(getHandler: 'setCtx -> 'entity -> HttpHandler) =
        { this with
            modifyDeleteSelfAcceptedResponse = getHandler
        }
...
type PolymorphicRelationshipHelper<'ctx, 'setCtx, 'entity, 'relatedEntity, 'relatedId> =
    internal
        {
            mapSetCtx: 'ctx -> 'entity -> Task<Result<'setCtx, Error list>>
            resolveEntity: ('relatedEntity -> PolymorphicBuilder<'ctx>) option
    ...
    member this.AddIdParser(resDef: ResourceDefinition<'ctx, 'e, 'id>, mapId: 'id -> 'relatedId) =
        { this with
            idParsers =
                this.idParsers
                |> Option.defaultValue Map.empty
                |> Map.add resDef.name (fun ctx resId -> resDef.id.toDomain ctx resId |> TaskResult.map mapId)
                |> Some
        }

    member this.ResolveEntity(getPolyBuilder: 'relatedEntity -> PolymorphicBuilder<'ctx>) =
        { this with
            resolveEntity = Some getPolyBuilder
        }
...
type RelationshipHelper<'ctx, 'setCtx, 'entity>
    ...
    member _.ToOne
        (
            resourceDef: ResourceDefinition<'ctx, 'relatedEntity, 'relatedId>,
