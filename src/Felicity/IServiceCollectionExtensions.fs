...
type JsonApiConfigBuilder<'ctx> = internal {
    ...
    /// from) the request. The returned HttpHandler may be used to modify the response (e.g. to set a header if deprecated
    /// fields are used).
    member this.TrackFieldUsageAsync
        (trackFieldUsage: IServiceProvider -> 'ctx -> FieldUseInfo list -> Async<HttpHandler>)
        : JsonApiConfigBuilder<'ctx> =
    ...
    /// from) the request. The returned HttpHandler may be used to modify the response (e.g. to set a header if deprecated
    /// fields are used).
    member this.TrackFieldUsage
        (trackFieldUsage: IServiceProvider -> 'ctx -> FieldUseInfo list -> HttpHandler)
        : JsonApiConfigBuilder<'ctx> =
    ...
    /// from) the request.
    member this.TrackFieldUsageAsync
        (trackFieldUsage: IServiceProvider -> 'ctx -> FieldUseInfo list -> Async<unit>)
        : JsonApiConfigBuilder<'ctx> =
        this.TrackFieldUsageTask(fun sp ctx xs -> trackFieldUsage sp ctx xs |> Task.fromAsync)
    ...
    /// from) the request.
    member this.TrackFieldUsage
        (trackFieldUsage: IServiceProvider -> 'ctx -> FieldUseInfo list -> unit)
        : JsonApiConfigBuilder<'ctx> =
        this.TrackFieldUsageTask(fun sp ctx xs -> trackFieldUsage sp ctx xs |> Task.result)
    ...
    member this.Add() =
        ...
        let getBaseUrl =
            ...
            | Some url, _ -> fun _ -> url

        let getCtx =
            this.getCtx
            |> Option.defaultWith (fun () -> failwith "Must specify a context getter")

        let configureSerializerOptions =
            this.configureSerializerOptions |> Option.defaultValue ignore
            ...
            let lookup =
                ...
                |> Array.collect (fun m ->
                    ...
                    let constraintsFieldByName =
                        ...
                        if hasConstrainedFields then
                            [| "constraints", typeof<obj> |]
                        else
                            [||]
        ...
        this.services
            ...
            .AddSingleton<JsonApiEndpoints<'ctx>>(
                JsonApiEndpoints(jsonApiEndpoints relativeRootWithLeadingSlash getCtx collections)
            )
            .AddSingleton<Serializer<'ctx>>(fun sp ->
                Serializer<'ctx>(
                    sp.GetRequiredService(),
                    this.invalidJsonRequestBodyLogLevel,
                    this.invalidJsonRequestBodyMaxSize,
                    sp.GetRequiredService(),
                    getFieldType,
            ...
            .AddSingleton<Serializer<ErrorSerializerCtx>>(fun sp ->
                Serializer<ErrorSerializerCtx>(
                    ...
                    None,
                    None,
                    UnknownFieldStrictMode<ErrorSerializerCtx>.Ignore,
                    getFieldType,
                    getFieldSerializationOrder,
                    configureSerializerOptions
                ))
...
module IServiceCollectionExtensions =
    ...
    type IServiceCollection with
        ...
        member this.AddJsonApi() =
            JsonApiConfigBuilder<'ctx>.DefaultFor this
