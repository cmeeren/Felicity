...
module HttpHandlers =
    ...
    let internal handleErrors (errs: Error list) : HttpHandler =
        fun (next: HttpFunc) (ctx: HttpContext) ->
            ...
            let headers = errs |> List.collect (fun e -> e.headers)
            ...
            let statusCode =
                ...
                | None ->
                    logger.LogWarning(
                        "None of the errors contained a valid integer status code; arbitrarily choosing status code 400"
                    )

                    400

            let handler = setStatusCode statusCode >=> jsonApiWithETag<ErrorSerializerCtx> doc

            handler next ctx


    let returnErrorDocument (errors: Error list) : HttpHandler = handleErrors errors


    let returnUnknownError: HttpHandler = returnErrorDocument [ unknownError () ]
