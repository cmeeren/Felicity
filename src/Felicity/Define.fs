...
type Define<'ctx, 'entity, 'id>() =
  member _.Attribute = AttributeHelper<'ctx, 'ctx, 'entity> (fun ctx _ -> Ok ctx |> Task.result)
  ...
  member _.Relationship = RelationshipHelper<'ctx, 'ctx, 'entity> (fun ctx _ -> Ok ctx |> Task.result)
