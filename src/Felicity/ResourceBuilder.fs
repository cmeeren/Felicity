...
type ResourceBuilder<'ctx>
    ...
    member _.Relationships(onlyData) = task {
...
let internal buildAndGetRelatedBuilders (builder: ResourceBuilder<'ctx>) = task {
...
let rec internal buildRecursive shouldBuildEntireResource addResource addRelationships (builder: ResourceBuilder<_>) = task {
...
let internal build (loggerFactory: ILoggerFactory) (mainBuilders: ResourceBuilder<'ctx> list) = task {
...
let internal buildOne loggerFactory (mainBuilder: ResourceBuilder<'ctx>) = task {
