...
let tests =
  testList "Validation" [
    ...
    testJob "Returns 406 if Accept is application/vnd.api+json but is modified with parameters" {
      ...
      let! json = response |> Response.readBodyAsString
      test <@ json |> getPath "errors[0].status" = "406" @>
      test <@ json |> getPath "errors[0].detail" = "The JSON:API media type in the Accept header must not be modified with media type parameters" @>
      test <@ json |> hasNoPath "errors[0].source" @>
      test <@ json |> hasNoPath "errors[1]" @>
    }
    ...
    testJob "Returns 415 if Content-Type is application/vnd.api+json but is modified with parameters" {
      let! response =
        Request.post Ctx "/as/1"
        |> Request.setHeader (ContentType (ContentType.create("application", "vnd.api+json", System.Text.Encoding.UTF8)))
        |> getResponse
      response |> testStatusCode 415
      let! json = response |> Response.readBodyAsString
      test <@ json |> getPath "errors[0].status" = "415" @>
      test <@ json |> getPath "errors[0].detail" = "The JSON:API media type in the Content-Type header must not be modified with media type parameters" @>
      test <@ json |> hasNoPath "errors[0].source" @>
      test <@ json |> hasNoPath "errors[1]" @>
    }
