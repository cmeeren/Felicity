...
module rec ResourceModules =
    ...
    module Parent =
        ...
        let lookup =
            define.Operation.Lookup(fun ctx pid -> ctx.Db.GetParentOrFail pid |> Some)
...
module rec ResourceModulesPreconditions =
    ...
    module Parent =
        ...
        let lookup =
            define.Operation.Lookup(fun ctx pid -> Db().GetParentOrFail("p1") |> Some)
    ...
    module Child =
        ...
        let post =
            define.Operation
                ...
                .PreconditionsETag(fun (_, p) -> EntityTagHeaderValue.FromString false p.ETag)
                .PreconditionsLastModified(fun (_, p) -> p.LastModified)
                .PreconditionsOptional.AfterCreate(ignore)
...
let tests =
    testList "POST collection backRef" [
        ...
        testJob "Returns error if collection case does not match" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "The path '/Children' does not exist, but differs only by case from the existing path '/children'. Paths are case sensitive."
                @>
