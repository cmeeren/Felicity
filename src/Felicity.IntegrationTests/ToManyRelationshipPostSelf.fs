...
module Parent7 = // ETag precondition
    ...
    let children =
        define.Relationship.ToMany(resDef).Get(fun ctx -> []).Add(fun ctx e -> e).AfterModifySelf(ignore)
...
module MapCtxCompileTest =
    ...
    let rel =
        define.Relationship
            ...
            .Add(fun (_: MappedCtx) _ _ -> failwith "never called")
    ...
    let withEntity =
        define.Relationship.MapSetContext(fun (ctx: Ctx6) (e: string) -> MappedCtx)
...
let tests =
    testList "To-many relationship POST self" [
        ...
        testJob "Parent1.children: Returns 200, modifies response, saves, and returns correct data if successful" {
            ...
            test <@ json |> hasNoPath "data[2].attributes" @>
            test <@ json |> hasNoPath "data[2].relationships" @>
            test <@ json |> hasNoPath "data[2].links" @>
            ...
            test <@ p.Id = "p1" @>
            test <@ p.Children = [ C1 { Id = "c1"; Child = { Id = "c3" } }; C2 { Id = "c2" }; C2 { Id = "c22" } ] @>
        }
        ...
        testJob "Supports include parameter and ignores include paths not starting with relationship name" {
            ...
            test <@ json |> getPath "included.[2].type" = "child2" @>
            test <@ json |> getPath "included.[2].id" = "c22" @>
            test <@ json |> getPath "included.[3].type" = "child3" @>
            test <@ json |> getPath "included.[3].id" = "c3" @>
            test <@ json |> getPath "included.[3].attributes.c" = "abc" @>
            test <@ json |> hasNoPath "included.[4]" @>
        }
        ...
        testJob "Returns 403 if relationship is not settable" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'children' on type 'parent3' does not support adding members using POST (other resource types in collection 'parents' may have a relationship called 'children' that supports POST)"
                @>
        ...
        testJob "Returns 403 if relationship is not settable but supports PATCH" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'supportsPatch' on type 'parent3' does not support adding members using POST; it supports PATCH to replace items (other resource types in collection 'parents' may have a relationship called 'supportsPatch' that supports POST)"
                @>
        ...
        testJob "Returns 403 if relationship is not settable but supports DELETE" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'supportsDelete' on type 'parent3' does not support adding members using POST; it supports DELETE to remove items (other resource types in collection 'parents' may have a relationship called 'supportsDelete' that supports POST)"
                @>
        ...
        testJob "Returns 403 if relationship is not settable but supports PATCH/DELETE" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'supportsPatchAndDelete' on type 'parent3' does not support adding members using POST; it supports PATCH/DELETE to replace/remove items (other resource types in collection 'parents' may have a relationship called 'supportsPatchAndDelete' that supports POST)"
                @>
        ...
        testJob "Returns 403 if relationship is not settable for any resource but supports PATCH" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'supportsPatchUnique' does not support adding members using POST for any resource in collection 'parents'; it may support PATCH to replace items"
                @>
        ...
        testJob "Returns 403 if relationship is not settable for any resource but supports DELETE" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'supportsDeleteUnique' does not support adding members using POST for any resource in collection 'parents'; it may support DELETE to remove items"
                @>
        ...
        testJob "Returns 403 if relationship is not settable for any resource but supports PATCH/DELETE" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'supportsPatchAndDeleteUnique' does not support adding members using POST for any resource in collection 'parents'; it may support PATCH/DELETE to replace/remove items"
                @>
        ...
        testJob "Returns 400 when missing body" {
            let db = Db()
            let! response = Request.post (Ctx.WithDb db) "/parents/p1/relationships/children" |> getResponse
...
"""
        ...
        testJob "Returns 403 if relationship exists but is not settable for any resource" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'children' does not support adding members using POST for any resource in collection 'parents'"
                @>
        ...
        testJob "Returns 404 if resource does not exist" {
            ...
            let! response =
                Request.post (Ctx.WithDb db) "/parents/invalidId/relationships/children"
                |> getResponse
        ...
        testJob "Returns error if collection case does not match" {
            let ctx = Ctx.WithDb(Db())
            let! response = Request.post ctx "/Parents/p1/relationships/children" |> getResponse
            response |> testStatusCode 404
            let! json = response |> Response.readBodyAsString
            test <@ json |> getPath "errors[0].status" = "404" @>
