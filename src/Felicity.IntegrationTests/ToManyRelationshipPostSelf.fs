...
type Db () =
  let mutable parents : Map<string, Parent> =
    Map.empty
    |> Map.add "p1" (P1 { Id = "p1"; Children = [C1 { Id = "c1"; Child = { Id = "c3" } }]; OtherChildIds = ["c2"] })
    |> Map.add "p3" (P3 { Id = "p3" })
    |> Map.add "p4" (P4 { Id = "p4" })
...
let tests =
  testList "To-many relationship POST self" [
    ...
    testJob "Calls AfterModifySelf with the initial and new entity" {
      ...
      let pExpected = { Id = "p1"; Children = [C1 { Id = "c1"; Child = { Id = "c3" } }; C2 { Id = "c2" }; C2 { Id = "c22" }]; OtherChildIds = ["c2"] }
    ...
    testJob "Related setter returns 404 for each related resource that is not found" {
      ...
      test <@ json |> getPath "errors[1].detail" = "The related resource with type 'child2' and ID 'c22' does not exist" @>
      test <@ json |> getPath "errors[1].source.pointer" = "/data/1" @>
      test <@ json |> hasNoPath "errors[2]" @>
    }
