...
let tests =
    testList "Routing" [
        ...
        testJob "Unknown link or relationship related URL gives 404" {
            let! response = Request.get Ctx "/as/ignoredId/unknownRel" |> getResponse
            response |> testStatusCode 404
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "The link or relationship 'unknownRel' does not exist for any resource in collection 'as'"
                @>
        ...
        testJob "Unknown relationship self URL gives 404" {
            let! response = Request.get Ctx "/as/ignoredId/relationships/unknownRel" |> getResponse
            response |> testStatusCode 404
            let! json = response |> Response.readBodyAsString
            test <@ json |> getPath "errors[0].status" = "404" @>
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "The relationship 'unknownRel' does not exist for any resource in collection 'as'"
                @>
        ...
        testJob "Unknown path after unknown relationship related URL gives 404" {
            let! response = Request.get Ctx "/as/ignoredId/unknownRel/path1/path2" |> getResponse
            response |> testStatusCode 404
            let! json = response |> Response.readBodyAsString
            test <@ json |> getPath "errors[0].status" = "404" @>
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "The path 'unknownRel/path1' does not exist for resources in collection 'as'"
                @>
        ...
        testJob "Unknown path after known relationship related URL gives 404" {
            let! response = Request.get Ctx "/as/ignoredId/knownRel/path1/path2" |> getResponse
            response |> testStatusCode 404
            let! json = response |> Response.readBodyAsString
            test <@ json |> getPath "errors[0].status" = "404" @>
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "The path 'knownRel/path1' does not exist for resources in collection 'as'"
                @>
        ...
        testJob "Unknown path after unknown relationship self URL gives 404" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "The path 'relationships/unknownRel/path1' does not exist for resources in collection 'as'"
                @>
        ...
        testJob "Unknown path after known relationship self URL gives 404" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "The path 'relationships/knownRel/path1' does not exist for resources in collection 'as'"
                @>
        ...
        testJob "Relative root path is case sensitive" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "The path '/Foo/bar/as/1' does not exist, but differs only by case from the existing path '/foo/bar/as/1'. Paths are case sensitive."
                @>
        ...
        testJob
            "If both base URL and relative root path is specified, uses the specified base URL in links regardless of actual request URL and root path, and makes the endpoints available at the root path" {
