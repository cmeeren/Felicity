...
module ChildDomain =
  ...
  let setName2 name2 child =
    { child with Name2 = name2 }
...
module GrandChild =
  ...
  let children =
    define.Relationship
      .ToMany(GreatGrandChild.resDef)
      .Get(fun c -> c.Children)
...
module Child =
  ...
  let child =
    define.Relationship
      .ToOneNullable(GrandChild.resDef)
      .Get(fun c -> c.Child)
...
let tests =
  testList "POST collection with included" [
        ...
        |> Request.bodySerialized
            {|data =
              ...
              included = [|
                ...
                {|``type`` = "child"
                  id = "ignoredId"
                  attributes = {| name = "childName"; name2 = "childName2" |}
                  relationships =
                    {|child =
                        {|data =
                            {|``type`` = "grandChild"
    ...
    testJob "Returns 400 when missing included resource, with included member" {
      ...
      let! json = response |> Response.readBodyAsString
      test <@ json |> getPath "errors[0].status" = "400" @>
      test <@ json |> getPath "errors[0].detail" = "Expected to find referenced resource with type 'child' and ID 'ignoredId' in the 'included' part of the request" @>
    ...
    testJob "Returns 409 if type is not allowed" {
            ...
            {|data =
              ...
              included = [|
                ...
                {|``type`` = "child"
                  ...
                  attributes = {| name = "childName" |}
                  relationships =
                    {|child = {| data = {| ``type`` = "invalid"; id = "foo" |} |} |}
                |} |> box
      ...
      test <@ json |> getPath "errors[0].detail" = "Relationship 'child' contains data with invalid type 'invalid'; expected 'grandChild'" @>
