...
let tests =
    testList "GET resource" [
        ...
        testJob "Returns 403 if missing lookup" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Collection 'as' does not support any resource-specific operations"
                @>
        ...
        testJob "Returns error if collection case does not match" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "The path '/As/1' does not exist, but differs only by case from the existing path '/as/1'. Paths are case sensitive."
                @>
