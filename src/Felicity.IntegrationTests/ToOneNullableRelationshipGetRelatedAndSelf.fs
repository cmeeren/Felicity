...
type Db () =
  let mutable parents : Map<string, Parent> =
    Map.empty
    |> Map.add "p1" (P1 { Id = "p1"; Child = C1 { Id = "c1" } })
...
let tests1 =
  testList "To-one nullable relationship GET related" [
    ...
    testJob "Parent1 null: Returns correct data if successful" {
      let db = Db ()
      let ctx = { Ctx.WithDb db with GetParent1Child = fun _ -> Include None }
      let! response = Request.get ctx "/parents/p1/child" |> getResponse
      response |> testStatusCode 200
      let! json = response |> Response.readBodyAsString
...
let tests2 =
  testList "To-one nullable relationship GET self" [
    ...
    testJob "Parent2: Returns 200, modifies response and returns correct data if successful" {
      ...
      test <@ json |> getPath "data.type" = "child2" @>
      test <@ json |> getPath "data.id" = "c2" @>
      test <@ json |> hasNoPath "data.id.attributes" @>
    ...
    testJob "Returns 400 if using include parameter" {
      let db = Db ()
      let! response = Request.get (Ctx.WithDb db) "/parents/p1/relationships/child?include=ignored" |> getResponse
