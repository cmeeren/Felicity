module ``To-one nullable relationship GET related and self``
...
type Ctx =
    {
        Db: Db
        ModifyGetRelatedResponse1: Parent1 -> Child option -> HttpHandler
        ModifyGetSelfResponse1: Parent1 -> Child option -> HttpHandler
        ModifyGetRelatedResponse2: Parent2 -> Child option -> HttpHandler
        ModifyGetSelfResponse2: Parent2 -> Child option -> HttpHandler
    ...
    static member WithDb db = {
        ...
        GetParent1Child = fun p -> Include(Some p.Child)
    }
...
module Parent1 =
    ...
    let child2 = define.Relationship.ToOne(Child2.resDef).Get(fun p -> p.Child2)
...
module Parent5 =
    ...
    let child: ToOneNullableRelationship<Ctx2, Ctx2, Parent4, obj, string> =
        define.Relationship.Polymorphic().ToOneNullable()
...
[<Tests>]
let tests1 =
    testList "To-one nullable relationship GET related" [

        testJob "Parent1 non-null: Returns 200, modifies response and returns correct data if successful" {
            let db = Db()
        ...
        testJob "Parent1 null: Returns correct data if successful" {
            let db = Db()

            let ctx =
                { Ctx.WithDb db with
                    GetParent1Child = fun _ -> Include None
                }
        ...
        testJob "Returns 403 if Skip" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "The server has chosen not to disclose the value of this relationship"
                @>
        ...
        testJob "Returns 403 if relationship exists but is not gettable for resource" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'child' on type 'parent3' is not readable (other resource types in collection 'parents' may have a readable relationship called 'child')"
                @>
        ...
        testJob "Returns 403 if relationship exists but is not gettable for any resource" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'child' is not readable for any resource in collection 'parents'"
                @>
        ...
        testJob "Returns error if collection case does not match" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "The path '/Parents/p1/child' does not exist, but differs only by case from the existing path '/parents/p1/child'. Paths are case sensitive."
                @>
...
[<Tests>]
let tests2 =
    testList "To-one nullable relationship GET self" [
        ...
        testJob "Supports include parameter and ignores include paths not starting with relationship name" {
            ...
            let! response =
                Request.get ctx "/parents/p1/relationships/child?include=child.subChild,child2"
                |> getResponse
        ...
        testJob "Returns 404 if resource does not exist" {
            ...
            let! response =
                Request.get (Ctx.WithDb db) "/parents/invalidId/relationships/child"
                |> getResponse
