...
module AB =
  ...
  let resId = define.Id.Simple(function A a -> a.Id | B b -> b.Id)
...
module AB2 =
  ...
  let resId = define.Id.ParsedOpt(id, (fun _ _ -> failwith "not used"), fun _ -> failwith "not used")
  let resDef =
    define.PolymorphicResource(resId)
      .CollectionName("abs")
