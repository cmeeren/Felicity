...
type Db () =
  let mutable parents : Map<string, Parent> =
    Map.empty
    |> Map.add "p1" (P1 { Id = "p1"; Children = [C1 { Id = "c1" }; C2 { Id = "c2" }]; OtherChildIds = ["c2"; "c1"] })
...
let tests1 =
  testList "To-many relationship PATCH resource" [
    ...
    testJob "Get/set: Returns 200, saves, and returns correct data if successful" {
      ...
      test <@ p.Id = "p1" @>
      test <@ p.Children = [C2 { Id = "c2" }; C1 { Id = "c1" }] @>
    ...
    testJob "Set-only: Returns 200, saves, and returns correct data if successful" {
      ...
      let p = 
        ...
        | _ -> failwith "not found"
      test <@ p.Id = "p2" @>
      test <@ p.Children = [C1 { Id = "c1" }; C2 { Id = "c2" }] @>
    }
...
let tests2 =
  testList "To-many relationship PATCH self" [
    ...
    testJob "Calls AfterModifySelf with the initial and new entity" {
      ...
      let pExpected = { Id = "p1"; Children = [C2 { Id = "c2" }; C1 { Id = "c1" }]; OtherChildIds = ["c2"; "c1"] }
    ...
    testJob "Returns 400 if using include parameter" {
      let db = Db ()
      let! response =
        Request.patch (Ctx.WithDb db) "/parents/p1/relationships/children?include=ignored"
        |> Request.bodySerialized
            {|data = [
              {|``type`` = "child2"; id = "c2" |}
              {|``type`` = "child1"; id = "c1" |}
