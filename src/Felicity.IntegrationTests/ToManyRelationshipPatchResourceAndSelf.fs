...
type Parent2 = { Id: string; Children: Child list }
...
module Parent5 =
    ...
    let children: ToManyRelationship<Ctx2, Ctx2, Parent4, obj, string> =
        define.Relationship.Polymorphic().ToMany()
...
module Parent7 = // ETag precondition
    ...
    let children =
        define.Relationship.ToMany(resDef).Get(fun ctx -> []).SetAll(fun ctx e -> e).AfterModifySelf(ignore)
...
let tests1 =
    testList "To-many relationship PATCH resource" [
            ...
            let! response =
                ...
                |> Request.bodySerialized {|
                    data = {|
                        ...
                        relationships = {|
                            children = {|
                                data = [ {| ``type`` = "child2"; id = "c2" |}; {| ``type`` = "child1"; id = "c1" |} ]
                            |}
                            otherChildren = {|
                                data = [ {| ``type`` = "child1"; id = "c1" |}; {| ``type`` = "child2"; id = "c2" |} ]
                            |}
                        |}
                    |}
                |}
        ...
        testJob "Related setter returns errors returned by setter" {
            ...
            let ctx = {
                Ctx.WithDb db with
                    SetChildren1 = fun _ _ -> Error [ Error.create 422 |> Error.setCode "custom" ]
            }
        ...
        testJob "Related setter returns 404 for each related resource that is not found" {
            ...
            test
                <@
                    json |> getPath "errors[1].detail" = "The related resource with type 'child1' and ID 'c1' does not exist"
                @>
        ...
        testJob "Related setter returns 404 for each related ID that fails to parse" {
            ...
            test
                <@
                    json |> getPath "errors[1].detail" = "The related resource with type 'child2' and ID 'c22' does not exist"
                @>
        ...
        testJob "ID setter returns errors returned by setter" {
            ...
            let ctx = {
                Ctx.WithDb db with
                    SetOtherChildrenIds1 = fun _ _ -> Error [ Error.create 422 |> Error.setCode "custom" ]
            }
            ...
            let! response =
                ...
                |> Request.bodySerialized {|
                    data = {|
                        ``type`` = "parent1"
                        id = "p1"
                        relationships = {| otherChildren = {| data = [] |} |}
                    |}
                |}
                |> getResponse
        ...
        testJob "Returns 403 if relationship is not settable but supports POST" {
            ...
            let! response =
                ...
                |> Request.bodySerialized {|
                    data = {|
                        ``type`` = "parent3"
                        id = "p3"
                        relationships = {| supportsPost = {| data = [] |} |}
                    |}
                |}
                |> getResponse
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'supportsPost' on type 'parent3' does not support complete replacement using PATCH; it supports POST to add items"
                @>
        ...
        testJob "Returns 403 if relationship is not settable but supports DELETE" {
            ...
            let! response =
                ...
                |> Request.bodySerialized {|
                    data = {|
                        ``type`` = "parent3"
                        id = "p3"
                        relationships = {| supportsDelete = {| data = [] |} |}
                    |}
                |}
                |> getResponse
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'supportsDelete' on type 'parent3' does not support complete replacement using PATCH; it supports DELETE to remove items"
                @>
        ...
        testJob "Returns 403 if relationship is not settable but supports POST/DELETE" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'supportsPostAndDelete' on type 'parent3' does not support complete replacement using PATCH; it supports POST/DELETE to add/remove items"
                @>
        ...
        testJob "Returns 400 if relationship is missing data" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'children' was specified without relationship data"
                @>
        ...
        testJob "Returns 409 for each type that is not allowed" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'children' contains data with invalid type 'invalid1'; expected one of 'child1', 'child2'"
                @>
            ...
            test
                <@
                    json |> getPath "errors[1].detail" = "Relationship 'children' contains data with invalid type 'invalid2'; expected one of 'child1', 'child2'"
                @>
            ...
            let! response =
                ...
                |> Request.bodySerialized {|
                    data = {|
                        ``type`` = "parent1"
                        id = "p1"
                        relationships = {| children = {| data = null |} |}
                    |}
                |}
                |> getResponse
...
let tests2 =
    testList "To-many relationship PATCH self" [
        ...
        testJob "Returns 403 if the relationship has no getter" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'children' on type 'parent2' is write-only and may only be updated through PATCH requests to the parent resource (other resource types in collection 'parents' may have a relationship called 'children' that supports this operation)"
                @>
        ...
        testJob "Related setter returns 404 for each related resource that is not found" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "The related resource with type 'child1' and ID 'c1' does not exist"
                @>
            ...
            test
                <@
                    json |> getPath "errors[1].detail" = "The related resource with type 'child2' and ID 'c2' does not exist"
                @>
        ...
        testJob "Returns 403 if relationship is not settable but supports POST" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'supportsPost' on type 'parent3' does not support complete replacement using PATCH; it supports POST to add items (other resource types in collection 'parents' may have a relationship called 'supportsPost' that supports PATCH)"
                @>
        ...
        testJob "Returns 403 if relationship is not settable but supports DELETE" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'supportsDelete' on type 'parent3' does not support complete replacement using PATCH; it supports DELETE to remove items (other resource types in collection 'parents' may have a relationship called 'supportsDelete' that supports PATCH)"
                @>
        ...
        testJob "Returns 403 if relationship is not settable but supports POST/DELETE" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'supportsPostAndDelete' on type 'parent3' does not support complete replacement using PATCH; it supports POST/DELETE to add/remove items (other resource types in collection 'parents' may have a relationship called 'supportsPostAndDelete' that supports PATCH)"
                @>
        ...
        testJob "Returns 403 if relationship is not settable for any resource but supports POST" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'supportsPostUnique' does not support PATCH for any resource in collection 'parents'; it may support POST to add items"
                @>
        ...
        testJob "Returns 403 if relationship is not settable for any resource but supports DELETE" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'supportsDeleteUnique' does not support PATCH for any resource in collection 'parents'; it may support DELETE to remove items"
                @>
        ...
        testJob "Returns 403 if relationship is not settable for any resource but supports POST/DELETE" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'supportsPostAndDeleteUnique' does not support PATCH for any resource in collection 'parents'; it may support POST/DELETE to add/remove items"
                @>
...
"""
        ...
        testJob "Returns 409 for each type that is not allowed" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Data contains invalid type 'invalid1'; expected one of 'child1', 'child2'"
                @>
            ...
            test
                <@
                    json |> getPath "errors[1].detail" = "Data contains invalid type 'invalid2'; expected one of 'child1', 'child2'"
                @>
        ...
        testJob "Returns 404 if relationship does not exist for resource" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'children' is not defined for resource type 'parent4' (other resource types in collection 'parents' may have a relationship called 'children')"
                @>
        ...
        testJob "Returns 403 if relationship exists but is not settable for any resource" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'children' does not support PATCH for any resource in collection 'parents'"
                @>
        ...
        testJob "Returns 404 if resource does not exist" {
            ...
            let! response =
                Request.patch (Ctx.WithDb db) "/parents/invalidId/relationships/children"
                |> getResponse
        ...
        testJob "Returns error if collection case does not match" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "The path '/Parents/p1/relationships/children' does not exist, but differs only by case from the existing path '/parents/p1/relationships/children'. Paths are case sensitive."
                @>
