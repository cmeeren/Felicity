...
module X =
    ...
    let relToManyGetAdd =
        define.Relationship
            .ToMany(A.resDef)
            .Get(fun _ _ -> failwith "not used")
            .Add(fun _ _ _ -> failwith "not used")
            .AfterModifySelf(ignore)

    let relToManyGetRemove =
        define.Relationship
            .ToMany(A.resDef)
            .Get(fun _ _ -> failwith "not used")
            .Remove(fun _ _ _ -> failwith "not used")
            .AfterModifySelf(ignore)
...
let tests =
    testList "Resource links" [
        ...
        testJob "To-one relationship self/related link should be present iff relationship has getter" {
            ...
            test
                <@
                    json |> getPath "data[0].relationships.relToOneGet.links.self" = "http://example.com/xs/someId/relationships/relToOneGet"
                @>

            test
                <@
                    json |> getPath "data[0].relationships.relToOneGet.links.related" = "http://example.com/xs/someId/relToOneGet"
                @>
            ...
            test
                <@
                    json |> getPath "data[0].relationships.relToOneGetSet.links.self" = "http://example.com/xs/someId/relationships/relToOneGetSet"
                @>

            test
                <@
                    json |> getPath "data[0].relationships.relToOneGetSet.links.related" = "http://example.com/xs/someId/relToOneGetSet"
                @>
        }
        ...
        testJob "To-one nullable relationship self/related link should be present iff relationship has getter" {
            ...
            test
                <@
                    json |> getPath "data[0].relationships.relToOneNullableGet.links.self" = "http://example.com/xs/someId/relationships/relToOneNullableGet"
                @>

            test
                <@
                    json |> getPath "data[0].relationships.relToOneNullableGet.links.related" = "http://example.com/xs/someId/relToOneNullableGet"
                @>
            ...
            test
                <@
                    json |> getPath "data[0].relationships.relToOneNullableGetSet.links.self" = "http://example.com/xs/someId/relationships/relToOneNullableGetSet"
                @>

            test
                <@
                    json |> getPath "data[0].relationships.relToOneNullableGetSet.links.related" = "http://example.com/xs/someId/relToOneNullableGetSet"
                @>
        }
        ...
        testJob "To-many relationship self/related link should be present iff relationship has getter" {
            ...
            test
                <@
                    json |> getPath "data[0].relationships.relToManyGet.links.self" = "http://example.com/xs/someId/relationships/relToManyGet"
                @>

            test
                <@
                    json |> getPath "data[0].relationships.relToManyGet.links.related" = "http://example.com/xs/someId/relToManyGet"
                @>
            ...
            test
                <@
                    json |> getPath "data[0].relationships.relToManyGetSet.links.self" = "http://example.com/xs/someId/relationships/relToManyGetSet"
                @>

            test
                <@
                    json |> getPath "data[0].relationships.relToManyGetSet.links.related" = "http://example.com/xs/someId/relToManyGetSet"
                @>

            test
                <@
                    json |> getPath "data[0].relationships.relToManyGetAdd.links.self" = "http://example.com/xs/someId/relationships/relToManyGetAdd"
                @>

            test
                <@
                    json |> getPath "data[0].relationships.relToManyGetAdd.links.related" = "http://example.com/xs/someId/relToManyGetAdd"
                @>

            test
                <@
                    json |> getPath "data[0].relationships.relToManyGetRemove.links.self" = "http://example.com/xs/someId/relationships/relToManyGetRemove"
                @>

            test
                <@
                    json |> getPath "data[0].relationships.relToManyGetRemove.links.related" = "http://example.com/xs/someId/relToManyGetRemove"
                @>
        }

        testJob
            "Relationships are not present if the resource does not have a collection name and the relationship is not included and does not have meta" {
            let! response = Request.get Ctx "/xs?include=a" |> getResponse
            response |> testSuccessStatusCode
            let! json = response |> Response.readBodyAsString
        ...
        testJob
            "Relationships are present with data and not links if the resource does not have a collection name and the relationship is included" {
        ...
        testJob
            "Relationships are present with data and not links if the resource has a collection name but not a get operation and the relationship is included" {
        ...
        testJob
            "Top-level relationships object is present if the resource has a collection name but not a get operation and no relationships are included" {
