...
module A =
    ...
    let resId =
        define.Id.ParsedOpt(id, (fun ctx s -> ctx.ParseId s), (fun (a: A) -> a.Id))
...
module B =
    ...
    let resId =
        define.Id.ParsedOpt(id, (fun ctx s -> ctx.ParseId s), (fun (b: B) -> b.Id))
...
let tests =
    testList "GET resource polymorphic" [
        ...
        testJob "Returns 403 if not supported for resource" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "GET is not supported for resource type 'c' (it may be supported for other resource types in collection 'abs')"
                @>
