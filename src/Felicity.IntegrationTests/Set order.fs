...
module X =
  ...
  let a =
    define.Attribute
      .SimpleString()
      .SetOrder(5)
      .Set(fun (Ctx xs) _ e -> xs := !xs @ [5]; e)

  let b =
    define.Attribute
      .SimpleString()
      .Set(fun (Ctx xs) _ e -> xs := !xs @ [0]; e)
  ...
  let i =
    define.Relationship
      .ToMany(resDef)
      .SetOrder(6)
      .SetAll(fun (Ctx xs) _ e -> xs := !xs @ [6]; e)
      .AfterModifySelf(ignore)

  let j =
    define.Relationship
      .ToMany(resDef)
      .SetAll(fun (Ctx xs) _ e -> xs := !xs @ [0]; e)
      .AfterModifySelf(ignore)
            ...
            {|data =
                {|``type`` = "a"
                  attributes =
                    ...
                    |}
                  relationships =
                    {|e = {| data = {| ``type`` = "a"; id = "ignored" |} |}
                      f = {| data = {| ``type`` = "a"; id = "ignored" |} |}
                      g = {| data = {| ``type`` = "a"; id = "ignored" |} |}
                      h = {| data = {| ``type`` = "a"; id = "ignored" |} |}
                      i = {| data = [ {| ``type`` = "a"; id = "ignored" |} ] |}
                      j = {| data = [ {| ``type`` = "a"; id = "ignored" |} ] |}
                    |}
                |}
            |}
        |> getResponse
