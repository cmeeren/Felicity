...
let tests1 =
    testList "To-many relationship GET related" [
        ...
        testJob "Returns 403 if relationship exists but is not gettable for resource" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'children' on type 'parent3' is not readable (other resource types in collection 'parents' may have a readable relationship called 'children')"
                @>
        ...
        testJob "Returns 403 if relationship exists but is not gettable for any resource" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Relationship 'children' is not readable for any resource in collection 'parents'"
                @>
        ...
        testJob "Returns error if collection case does not match" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "The path '/Parents/p1/children' does not exist, but differs only by case from the existing path '/parents/p1/children'. Paths are case sensitive."
                @>
...
let tests2 =
    testList "To-many relationship GET self" [
        ...
        testJob "Supports include parameter and ignores include paths not starting with relationship name" {
            ...
            let! response =
                Request.get ctx "/parents/p1/relationships/children?include=children.subChild,child2"
                |> getResponse
        ...
        testJob "Returns 404 if resource does not exist" {
            ...
            let! response =
                Request.get (Ctx.WithDb db) "/parents/invalidId/relationships/children"
                |> getResponse
