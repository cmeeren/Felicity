module ``To-one nullable relationship PATCH resource and self``
...
type Db () =
  let mutable parents : Map<string, Parent> =
    Map.empty
    |> Map.add "p1" (P1 { Id = "p1"; Child = Some (C1 { Id = "c1"; Child = { Id = "c3" } }); OtherChildId = Some "c2"; NullableNotNullWhenSet = None })
...
type Ctx = {
  Db: Db
  ModifyPatchSelfOkResponse: Parent1 -> Child option -> HttpHandler
  ModifyPatchSelfAcceptedResponse: Parent1 -> HttpHandler
  GetParent1Child: Parent1 -> Child option Skippable
  SetChild1: Child option -> Parent1 -> Result<Parent1, Error list>
  SetOtherChildId1: string option -> Parent1 -> Result<Parent1, Error list>
...
module Parent1 =  // set and get - PATCH resource/self OK
  ...
  let otherChild =
    define.Relationship
      ...
      .ToOneNullable()
      .Get(fun ctx p -> p.OtherChildId |> Option.map (ctx.Db.TryGetChild >> Option.defaultWith (fun () -> failwith "Not found")))
...
module MapCtxCompileTest =
  ...
  let withEntity =
    define.Relationship
      .MapSetContext(fun (ctx: Ctx7) (e: string) -> MappedCtx)



[<Tests>]
let tests1 =
  testList "To-one nullable relationship PATCH resource" [
    ...
    testJob "Get/set non-null: Returns 200, saves, and returns correct data if successful" {
      ...
      test <@ p.NullableNotNullWhenSet = Some (C1 { Id = "c1"; Child = { Id = "c3" } }) @>
    }
    ...
    testJob "Get/set null: Returns 200, saves, and returns correct data if successful" {
      ...
      test <@ json |> getPath "data.type" = "parent1" @>
      test <@ json |> getPath "data.id" = "p1" @>
      test <@ json |> getPath "data.relationships.child.data" = null @>
      test <@ json |> getPath "data.relationships.otherChild.data" = null @>
    ...
    testJob "Set-only: Returns 200, saves, and returns correct data if successful" {
      ...
      let p = 
        ...
        | _ -> failwith "not found"
      test <@ p.Id = "p2" @>
      test <@ p.Child = Some (C1 { Id = "c1"; Child = { Id = "c3" } }) @>
    }
    ...
    testJob "Related setter returns errors returned by setter" {
      let db = Db ()
      let ctx = { Ctx.WithDb db with SetChild1 = fun _ _ -> Error [Error.create 422 |> Error.setCode "custom"] }
      let! response =
        Request.patch ctx "/parents/p1"
        |> Request.bodySerialized
            {|data =
    ...
    testJob "ID setter returns errors returned by setter" {
      let db = Db ()
      let ctx = { Ctx.WithDb db with SetOtherChildId1 = fun _ _ -> Error [Error.create 422 |> Error.setCode "custom"] }
      let! response =
        Request.patch ctx "/parents/p1"
        |> Request.bodySerialized
            {|data =
    ...
    testJob "Returns 400 if relationship is missing data" {
      ...
      test <@ json |> getPath "errors[0].detail" = "Relationship 'child' was specified without relationship data" @>
      test <@ json |> getPath "errors[0].source.pointer" = "/data/relationships/child" @>
      test <@ json |> hasNoPath "errors[1]" @>
    }
    ...
    testJob "Returns 409 if type is not allowed" {
      ...
      test <@ json |> getPath "errors[0].detail" = "Relationship 'child' contains data with invalid type 'invalid'; expected one of 'child1', 'child2'" @>
...
[<Tests>]
let tests2 =
  testList "To-one nullable relationship PATCH self" [

    testJob "Parent1.child non-null: Returns 200, modifies response, saves, and returns correct data if successful" {
      let db = Db ()
    ...
    testJob "Parent1.child null: Saves and returns correct data if successful" {
      let db = Db ()
      let! response =
    ...
    testJob "Parent1.otherChild non-null: Returns 202, modifies response, saves, and returns correct data if successful" {
      let db = Db ()
    ...
    testJob "Parent1.otherChild null: Returns 202, saves, and returns correct data if successful" {
      let db = Db ()
      let! response =
        Request.patch (Ctx.WithDb db) "/parents/p1/relationships/otherChild"
        |> Request.bodySerialized {| data = null |}
        |> getResponse
      response |> testStatusCode 202
      let! json = response |> Response.readBodyAsString
    ...
    testJob "Returns 403 if the relationship has no getter" {
      ...
      let! json = response |> Response.readBodyAsString
      test <@ json |> getPath "errors[0].status" = "403" @>
      test <@ json |> getPath "errors[0].detail" = "Relationship 'child' on type 'parent2' is write-only and may only be updated through PATCH requests to the parent resource (other resource types in collection 'parents' may have a relationship called 'child' that supports this operation)" @>
      test <@ json |> hasNoPath "errors[0].source" @>
      test <@ json |> hasNoPath "errors[1]" @>
    }
    ...
    testJob "Returns 403 if relationship is not settable" {
      ...
      let! response =
        ...
        |> Request.bodySerialized {| data = {| ``type`` = "child2"; id = "c2" |} |}
        |> getResponse
      response |> testStatusCode 403
      let! json = response |> Response.readBodyAsString
      test <@ json |> getPath "errors[0].status" = "403" @>
...
"""
    ...
    testJob "Returns 409 if type is not allowed" {
      ...
      let! response =
        Request.patch (Ctx.WithDb db) "/parents/p1/relationships/child"
        |> Request.bodySerialized {| data = {| ``type`` = "invalid"; id = "foo" |} |}
        |> getResponse
      response |> testStatusCode 409
      let! json = response |> Response.readBodyAsString
      test <@ json |> getPath "errors[0].status" = "409" @>
      test <@ json |> getPath "errors[0].detail" = "Data contains invalid type 'invalid'; expected one of 'child1', 'child2'" @>
      test <@ json |> getPath "errors[0].source.pointer" = "/data/type" @>
      test <@ json |> hasNoPath "errors[1]" @>
    }
    ...
    testJob "Returns 403 if relationship exists but is not settable for any resource" {
      ...
      let! json = response |> Response.readBodyAsString
      test <@ json |> getPath "errors[0].status" = "403" @>
      test <@ json |> getPath "errors[0].detail" = "Relationship 'child' does not support PATCH for any resource in collection 'parents'" @>
      test <@ json |> hasNoPath "errors[0].source" @>
      test <@ json |> hasNoPath "errors[1]" @>
    }
    ...
    testJob "Returns 404 if resource does not exist" {
      let db = Db ()
      let! response = Request.patch (Ctx.WithDb db) "/parents/invalidId/relationships/child" |> getResponse
      response |> testStatusCode 404
      let! json = response |> Response.readBodyAsString
      test <@ json |> getPath "errors[0].status" = "404" @>
    ...
    testJob "Returns 403 if missing lookup" {
      let! response = Request.patch Ctx3 "/parents/invalidId/relationships/child" |> getResponse
