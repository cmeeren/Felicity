...
type Parent2 = { Id: string; Child: Child option }
...
type Db() =
    let mutable parents: Map<string, Parent> =
        ...
        |> Map.add
            ...
            (P1 {
                Id = "p1"
                Child = Some(C1 { Id = "c1"; Child = { Id = "c3" } })
                OtherChildId = Some "c2"
...
let tests1 =
    testList "To-one nullable relationship PATCH resource" [
        ...
        testJob "Returns 403 if set to null when not supported" {
            ...
            test
                <@ json |> getPath "errors[0].detail" = "Relationship 'nullableNotNullWhenSet' may not be set to null" @>
...
let tests2 =
    testList "To-one nullable relationship PATCH self" [
        ...
        testJob
            "Parent1.otherChild non-null: Returns 202, modifies response, saves, and returns correct data if successful" {
            let db = Db()
