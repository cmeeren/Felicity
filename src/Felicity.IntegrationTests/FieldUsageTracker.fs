...
module C =
  ...
  let post = define.Operation.Post(fun () -> C "1").Return202Accepted().AfterCreate(ignore)
...
module D =
  ...
  let postCustom202 =
    define.Operation
      .PostCustomAsync(fun _ parser helper ->
        async {
          let parser = parser.For((fun _ _ -> D "1"), resId, c)
          helper.ValidateRequest(parser) |> function Ok x -> x | Error _ -> failwith "Should not happen"
          let! d = parser.ParseAsync() |> Async.map (function Ok x -> x | Error _ -> failwith "Should not happen")
          let! _d = helper.RunSettersAsync(d, parser) |> Async.map (function Ok x -> x | Error _ -> failwith "Should not happen")
    ...
    define.Operation
      .PostCustomAsync(fun _ parser helper ->
        async {
          ...
          helper.ValidateRequest(parser) |> function Ok x -> x | Error _ -> failwith "Should not happen"
          let! b = parser.ParseAsync() |> Async.map (function Ok x -> x | Error _ -> failwith "Should not happen")
          let! b = helper.RunSettersAsync(b, parser) |> Async.map (function Ok x -> x | Error _ -> failwith "Should not happen")
...
module A =
  ...
  let attrCAndDSetter =
    define.Operation.Set2((fun _ _ a -> a), attrC, attrD)
  ...
  let relCDToOne =
    define.Relationship
      ...
      .ResolveEntity(function CDC c -> C.resDef.PolymorphicFor(c) | CDD d -> D.resDef.PolymorphicFor(d))
      .ToOne()
      .Get(fun _ -> CDC (C "1"))
      .Set(fun _ a -> a)
      .AfterModifySelf(ignore)
...
module CD =
  ...
  let resId = define.Id.Simple(function CDC (C id) -> id | CDD (D id) -> id)
  ...
  let lookup =
    define.Operation
      ...
      .Lookup(
        ...
        function CDC c -> C.resDef.PolymorphicFor(c) | CDD d -> D.resDef.PolymorphicFor(d)
      )
...
type GH = G of string | H of string


module GH =

  let define = Define<Ctx, GH, string>()
  let resId = define.Id.Simple(function G g -> g | H h -> h)
  ...
  let lookup =
    define.Operation
      ...
      .Lookup(
        ...
        function G g -> G.resDef.PolymorphicFor(g) | H h -> H.resDef.PolymorphicFor(h)
      )
...
let doTest method url body expected =
    job {
      ...
      let! response =
        ...
        |> match body with None -> id | Some body -> Request.bodySerialized body
        |> getResponse
...
let tests =
  testList "Field usage tracker" [
    ...
    let basicTestFieldsAndIncludesForPrimaryA = "include=relB,relBNone&fields[a]=attrA,relB,constraints"
    ...
    testJob "GET polymorphic collection: Tracks fields and includes from all possible types" {
      do!
        doTest
          Get
          "http://example.com/cds?include=relToOneNullable&fields[c]=c,relToOne"
          None
          [
    ...
    testJob "GET polymorphic collection when including polymorphic relationship: Tracks correct usage for all related resource types" {
      do!
        doTest
          Get
          "http://example.com/ghs?include=r"
          None
          [
    ...
    testJob "GET polymorphic collection when resources belong to another collection: Tracks correct usage for all related resource types" {
      do!
        doTest
          Get
          "http://example.com/ghsAlt"
          None
          [
    ...
    testJob "POST collection: Tracks request fields" {
      do!
        doTest
          Post
          "http://example.com/as"
          (Some {| data = {| ``type`` = "a"; attributes = {| attrA = "" |} |} |})
          [
            ("a", "attrA"), FieldUsage.Explicit
    ...
    testJob "POST collection 202: Tracks only request fields" {
      do!
        doTest
          ...
          (Some {| data = {| ``type`` = "c"; relationships = {| relToOneNullable = {| data = null |} |} |} |})
          [
            ("c", "relToOneNullable"), FieldUsage.Explicit
          ]
    }
    ...
    testJob "POST collection custom: Tracks request fields" {
      do!
        doTest
          Post
          "http://example.com/bs"
          (Some {| data = {| ``type`` = "b"; attributes = {| a = ""; b = "" |} |} |})
          [
            ("b", "a"), FieldUsage.Explicit
    ...
    testJob "POST collection custom 202: Tracks only request fields, including ID" {
      do!
        doTest
          Post
          "http://example.com/cds"
          (Some {| data = {| ``type`` = "d"; id = "1"; attributes = {| c = "" |} |} |})
          [
            ("d", "id"), FieldUsage.Explicit
            ("d", "c"), FieldUsage.Explicit
          ]
    }
    ...
    testJob "POST collection backRef: Tracks backreference" {
      do!
        doTest
          Post
          "http://example.com/es"
          (Some {| data = {| ``type`` = "e"; relationships = {| r = {| data = null |} |} |} |})
          [
            ("e", "a"), FieldUsage.Implicit
            ("e", "r"), FieldUsage.Explicit
          ]
    }
    ...
    testJob "POST collection backRef parser overload: Tracks backreference" {
      do!
        doTest
          Post
          "http://example.com/fs"
          (Some {| data = {| ``type`` = "f"; relationships = {| r = {| data = null |} |} |} |})
          [
            ("f", "a"), FieldUsage.Implicit
            ("f", "r"), FieldUsage.Explicit
          ]
    }
    ...
    testJob "PATCH resource: Tracks request fields, including custom setters and Set2" {
      do!
        doTest
          ...
          (Some {| data = {| ``type`` = "a"; id = "1"; attributes = {| attrA = ""; attrB = ""; attrC = ""; attrD = "" |}; relationships = {| relBNone = {| data = null |} |} |} |})
          [
            ("a", "attrA"), FieldUsage.Explicit
    ...
    testJob "PATCH resource 202: Tracks only request fields" {
      do!
        doTest
          ...
          (Some {| data = {| ``type`` = "e"; id = "1"; attributes = {| a = "" |} |} |})
          [
    ...
    testJob "Custom operation in polymorphic collection: Only tracks the requested type" {
      do!
        doTest
          Get
          "http://example.com/ghs/G1/customOp"
          None
          [
            ("g", "r"), FieldUsage.Implicit
          ]
    ...
    testJob "GET to-one polymorphic relationship related: Tracks correct usage for all related resource types" {
      do!
        doTest
          Get
          "http://example.com/as/1/relCDToOne"
          None
          [
    ...
    testJob "GET to-one relationship related in polymorphic collection: Tracks only the relationship on the requested type" {
      do!
        doTest
          Get
          "http://example.com/cds/C1/relToOne"
          None
          [
            ("c", "c"), FieldUsage.Implicit
            ("c", "relToOne"), FieldUsage.Explicit
    ...
    testJob "GET to-one relationship self in polymorphic collection: Tracks only the relationship on the requested type" {
      do!
        doTest
          Get
    ...
    testJob "PATCH to-one relationship self in polymorphic collection: Tracks only the relationship on the requested type" {
      do!
        doTest
          Patch
          "http://example.com/cds/C1/relationships/relToOne"
    ...
    testJob "GET to-one nullable polymorphic relationship related: Tracks correct usage for all related resource types" {
      do!
        doTest
          Get
          "http://example.com/as/1/relCDToOneNullable"
          None
          [
    ...
    testJob "GET to-one nullable relationship related in polymorphic collection: Tracks only the relationship on the requested type" {
      do!
        doTest
          Get
          "http://example.com/cds/C1/relToOneNullable"
          None
          [
            ("c", "c"), FieldUsage.Implicit
            ("c", "relToOne"), FieldUsage.Implicit
    ...
    testJob "GET to-one nullable relationship self in polymorphic collection: Tracks only the relationship on the requested type" {
      do!
        doTest
          Get
          "http://example.com/cds/C1/relationships/relToOneNullable"
          None
          [
            ("c", "relToOneNullable"), FieldUsage.Explicit
          ]
    ...
    testJob "PATCH to-one nullable polymorphic relationship self: Tracks correct usage for all related resource types" {
      do!
        doTest
          Patch
          "http://example.com/as/1/relationships/relCDToOneNullable?include=relCDToOneNullable"
    ...
    testJob "PATCH to-one nullable relationship self in polymorphic collection: Tracks only the relationship on the requested type" {
      do!
        doTest
          Patch
          "http://example.com/cds/C1/relationships/relToOneNullable"
    ...
    testJob "GET to-many polymorphic relationship related: Tracks correct usage for all related resource types" {
      do!
        doTest
          Get
          "http://example.com/as/1/relCDToMany"
          None
          [
    ...
    testJob "GET to-many relationship related in polymorphic collection: Tracks only the relationship on the requested type" {
      do!
        doTest
          Get
          "http://example.com/cds/C1/relToMany"
          None
          [
            ("c", "c"), FieldUsage.Implicit
            ("c", "relToOne"), FieldUsage.Implicit
    ...
    testJob "GET to-many relationship self in polymorphic collection: Tracks only the relationship on the requested type" {
      do!
        doTest
          Get
          "http://example.com/cds/C1/relationships/relToMany"
          None
          [
            ("c", "relToMany"), FieldUsage.Explicit
          ]
    ...
    testJob "POST to-many relationship self in polymorphic collection: Tracks only the relationship on the requested type" {
      do!
        doTest
          Post
          "http://example.com/cds/C1/relationships/relToMany"
    ...
    testJob "PATCH to-many relationship self in polymorphic collection: Tracks only the relationship on the requested type" {
      do!
        doTest
          Patch
          "http://example.com/cds/C1/relationships/relToMany"
    ...
    testJob "DELETE to-many relationship self in polymorphic collection: Tracks only the relationship on the requested type" {
      do!
        doTest
          Delete
          "http://example.com/cds/C1/relationships/relToMany"
    ...
    testJob "Works across null relationships" {
      do!
        doTest
          Get
          "http://example.com/as/1?include=relBNone"
          None
          [
    ...
    testJob "Works across empty relationships" {
      do!
        doTest
          Get
          "http://example.com/as/1?include=relBEmpty"
          None
          [
            ("a", "attrA"), FieldUsage.Implicit
            ("a", "relCDToOne"), FieldUsage.Implicit
    ...
    testJob "Ignores types that don't exist" {
      do!
        doTest
          Get
          "http://example.com/as/1?fields[invalidType]=foo"
          None
          [
            ("a", "attrA"), FieldUsage.Implicit
            ("a", "relCDToOne"), FieldUsage.Implicit
    ...
    testJob "Ignores fields that don't exist" {
      do!
        doTest
          Get
          "http://example.com/as/1?fields[a]=invalidField"
          None
          [
    ...
    testJob "Ignores fields on types that are not included" {
      do!
        doTest
          Get
          "http://example.com/as/1?fields[b]=a&fields[c]=c"
          None
          [
            ("a", "attrA"), FieldUsage.Implicit
            ("a", "relCDToOne"), FieldUsage.Implicit
