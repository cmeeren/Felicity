...
type Db() =
    ...
    member _.Remove id = ABs <- ABs.Remove id
...
let tests =
    testList "DELETE resource" [
        ...
        testJob "Returns 403 if not supported by resource" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "DELETE is not supported for resource type 'c' (it may be supported for other resource types in collection 'abs')"
                @>
        ...
        testJob "Returns 403 if not supported at all" {
            ...
            test
                <@ json |> getPath "errors[0].detail" = "DELETE is not supported for any resource in collection 'abs'" @>
