...
"""
    ...
    testJob "Returns 400 when setting a non-nullable DateTimeOffset attribute without offset" {
      ...
      let! json = response |> Response.readBodyAsString
      test <@ json |> getPath "errors[0].status" = "400" @>
      test <@ json |> getPath "errors[0].detail" = "Attribute 'nonNullable' got invalid value '2000-01-01T15:49:23': The value must be a valid ISO 8601-1:2019 date-time including an offset (e.g. 'Z' or '+01:00')" @>
    ...
    testJob "Returns 400 when setting a nullable DateTimeOffset attribute without offset" {
      ...
      let! json = response |> Response.readBodyAsString
      test <@ json |> getPath "errors[0].status" = "400" @>
      test <@ json |> getPath "errors[0].detail" = "Attribute 'nullable' got invalid value '2000-01-01T15:49:23': The value must be a valid ISO 8601-1:2019 date-time including an offset (e.g. 'Z' or '+01:00')" @>
