...
module BDomain =
    ...
    let setB x b = { b with B = x }

    let setY x b = { b with Y = x }
...
type Db() =
    ...
    member _.SaveB(b: B) = ABs <- ABs.Add(b.Id, B b)

    member _.TryGet id = ABs.TryFind id

    member _.GetOrFail id =
        ABs.TryFind id
        |> Option.defaultWith (fun () -> failwith $"Could not find ID %s{id}")
...
module B =
    ...
    let readonly = define.Attribute.SimpleString().Get(fun b -> b.ReadOnly)

    let b = define.Attribute.SimpleInt().Get(fun b -> b.B).Set(BDomain.setB)
...
module A5 =
    ...
    let preconditions =
        define.Preconditions.LastModified(fun _ -> DateTimeOffset(2000, 1, 1, 0, 0, 0, 999, TimeSpan.Zero)) // Should ignore milliseconds
    ...
    testList "PATCH resource" [
        ...
        testJob "Calls AfterUpdate with the initial and new entity" {
            ...
            let aExpected = {
                ...
                X = "abc"
                Nullable = None
                NullableNotNullWhenSet = Some "bar"
            }
        ...
        testJob "Returns 400 when missing body" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Request to update resources must contain a single resource object as primary data"
                @>
...
"""
        ...
        testJob "Returns 400 when setting a non-nullable DateTimeOffset attribute without offset" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Attribute 'nonNullable' got invalid value '2000-01-01T15:49:23': The value must be a valid ISO 8601-1:2019 date-time including an offset (e.g. 'Z' or '+01:00')"
                @>
        ...
        testJob "Returns 400 when setting a nullable DateTimeOffset attribute without offset" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Attribute 'nullable' got invalid value '2000-01-01T15:49:23': The value must be a valid ISO 8601-1:2019 date-time including an offset (e.g. 'Z' or '+01:00')"
                @>
        ...
        testJob "Can set DateTimeOffset attributes with offset" {
            ...
            test
                <@ json |> getPath "data.attributes.nonNullable" = DateTimeOffset(2000, 1, 1, 15, 49, 23, TimeSpan.Zero) @>

            test
                <@
                    json |> getPath "data.attributes.nullable" = DateTimeOffset(
                        2000,
                        1,
                        1,
                        15,
        ...
        testJob "Returns 403 if not supported by resource" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "PATCH is not supported for resource type 'c' (it may be supported for other resource types in collection 'abs')"
                @>
        ...
        testJob "Returns 403 if not supported at all" {
            ...
            test
                <@ json |> getPath "errors[0].detail" = "PATCH is not supported for any resource in collection 'abs'" @>
        ...
        testJob "Returns 403 if missing lookup" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "Collection 'abs' does not support any resource-specific operations"
                @>
        ...
        testJob "Returns error if collection case does not match" {
            ...
            test
                <@
                    json |> getPath "errors[0].detail" = "The path '/Abs/a1' does not exist, but differs only by case from the existing path '/abs/a1'. Paths are case sensitive."
                @>
