...
let tests =
  testList "Skip links" [
    ...
    testJob "Correctly relationship data but not links if relationships are included and links are skipped" {
      ...
      let! response =
        Request.createWithClient client Get (Uri("http://example.com/as/1?include=toOne,toOneNullable,toMany&skipStandardLinks"))
        |> Request.jsonApiHeaders
        |> getResponse
      response |> testSuccessStatusCode
      let! json = response |> Response.readBodyAsString
    ...
    testJob "Query parameter values cause errors" {
      ...
      let! response =
        Request.createWithClient client Get (Uri("http://example.com/as/1?skipStandardLinks=false&skipCustomLinks=0&skipStandardLinks=foo"))
        |> Request.jsonApiHeaders
        |> getResponse
      response |> testStatusCode 400
      let! json = response |> Response.readBodyAsString
      Expect.equal (json |> getPath "errors[0].status") "400" ""
      Expect.equal (json |> getPath "errors[0].detail") "Query parameter 'skipStandardLinks' must be specified without a value, but got 'false'" ""
      Expect.equal (json |> getPath "errors[0].source.parameter") "skipStandardLinks" ""
      Expect.equal (json |> getPath "errors[1].status") "400" ""
      Expect.equal (json |> getPath "errors[1].detail") "Query parameter 'skipCustomLinks' must be specified without a value, but got '0'" ""
