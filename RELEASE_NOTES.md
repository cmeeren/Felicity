...
* Now supports strict mode validation of query parameters in custom operations using `ValidateStrictModeQueryParams` (search the documentation for details)
...
* Improved errors returned when parsing values in query, header, or body:
  ...
  * Parse error messages for query parameters, headers, and resource IDs defined with overloads that return user-defined error messages (e.g. `Result<_, string>`) are generally of the form `Query parameter 'name' got invalid value 'invalidValue': <User-defined error message>`
